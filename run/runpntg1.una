...
#
set -x
arqfiles=`ls ${ROPERM}/model/dataout/${CASE}/${LABELI}/GFCT${prefx}${LABELI}${LABELF}*.files`

if [  -s $arqfiles ]; then
      echo "\n+++ Rodando Modelo AGCMCPT ${CASE}"
      echo "   runModel 20 4 ENSMODEL ${TRC} ${LV} ${LABELI}  ${LABELF} hold"
${OPERM}/run/runModel 16 2 1 ENSMODEL${PERT} ${TRC} ${LV} ${LABELI}  ${LABELF} ${PERT} hold
else
...
${OPERM}/run/runGrh  1 1 EnsGRH ${TRC} ${LV}   ${LABELI} ${LABELF}  GFGN${PERT} psnm ${PERT} hold
exit 0
### F I M ###
