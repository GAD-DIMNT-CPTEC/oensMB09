PROGRAM ensmed
  ...
  CHARACTER(LEN=1024)              :: dirin
  CHARACTER(LEN=1024)              :: dirout
  CHARACTER(LEN=512)              :: dirctl
  CHARACTER(LEN=512)              :: ensmedi
  INTEGER                         :: ierr
  INTEGER                         :: imax
  INTEGER                         :: jmax
  INTEGER                         :: nbs
  ...
  CHARACTER(LEN=11)              :: prefx,resol
  CHARACTER(LEN=512)              :: lstoutlst
  INTEGER                         :: numproc
  INTEGER                         :: thisproc
  INTEGER                         :: firstcount
  INTEGER                         :: lastcount
  ...
  CHARACTER(LEN= 10), ALLOCATABLE :: fctdate  (:)
  CHARACTER(LEN=512), ALLOCATABLE :: lstoutctl(:)
  CHARACTER(LEN=512), ALLOCATABLE :: lstoutgrb(:)
  CHARACTER(LEN=512), ALLOCATABLE :: lstinctl (:,:)
  CHARACTER(LEN=512), ALLOCATABLE :: lstingrb (:,:)
  REAL              , ALLOCATABLE :: tmp(:)
  REAL              , ALLOCATABLE :: varsmed(:)
  ...
  !
  WRITE(*,*) 'File 10: '//TRIM(ensmedi)
  OPEN(10,FILE=TRIM(ensmedi),STATUS='OLD',FORM='FORMATTED')
  CALL attribute(10,nbyte,nfctdy,freqcalc,nmembers,imax,jmax,dirin,dirout,resol,prefx) 
  CLOSE(10)
...
!CFB           
     ...
     END DO
  END DO
  IF (ALLOCATED(fctdate)  ) DEALLOCATE(fctdate  )
  IF (ALLOCATED(lstoutctl)) DEALLOCATE(lstoutctl)
  IF (ALLOCATED(lstoutgrb)) DEALLOCATE(lstoutgrb)
  IF (ALLOCATED(lstinctl) ) DEALLOCATE(lstinctl )
  IF (ALLOCATED(lstingrb) ) DEALLOCATE(lstingrb )
  IF (ALLOCATED(tmp)      ) DEALLOCATE(tmp      )
  IF (ALLOCATED(varsmed)  ) DEALLOCATE(varsmed  )
