...
MODULE Utils
  ...
  REAL, ALLOCATABLE :: colrad2D(:,:)
  REAL, ALLOCATABLE :: cos2lat(:,:)
  REAL, ALLOCATABLE :: ercossin(:,:)
  REAL, ALLOCATABLE :: fcor(:,:)
  REAL, ALLOCATABLE :: cosiv(:,:)
  REAL, ALLOCATABLE :: coslatj(:)
  REAL, ALLOCATABLE :: sinlatj(:)
  REAL, ALLOCATABLE :: coslat(:,:)
  REAL, ALLOCATABLE :: sinlat(:,:)
  REAL, ALLOCATABLE :: coslon(:,:)
  REAL, ALLOCATABLE :: sinlon(:,:)
  REAL, ALLOCATABLE :: longit(:,:)
  REAL, ALLOCATABLE :: rcl(:,:)
  REAL, ALLOCATABLE :: lonrad(:,:)
  REAL, ALLOCATABLE :: long(:,:)
  REAL, ALLOCATABLE :: lati(:,:)
  REAL, ALLOCATABLE :: cosz(:,:)
  REAL, ALLOCATABLE :: cos2d(:,:)
  REAL, ALLOCATABLE :: vmax(:,:)
  REAL, ALLOCATABLE :: vmaxVert(:)
...
CONTAINS
  ...
  SUBROUTINE CreateGaussQuad (degreeGiven, pointsGiven, weightsGiven)
    ...
    CHARACTER(LEN=10) :: c1
    INTEGER :: j
    !**(JP)** altero pi para aproximar resultados deste modelo com os resultados do anterior
    REAL :: mypai=3.1415926e0 !**(JP)** mesmo pi usado no modelo anterior neste ponto do programa
  ...
  DO j=1,maxDegree/2
     ...
     colrad(maxDegree+1-j)=mypai-GaussColat(j)
  ...
  SUBROUTINE CreateGridValues
    ...
    DO j=1,jMax
       ...
       IF (j.eq.jhalf) THEN 
         ...
         ELSE
          sinlatj (j)= - sinlatj(jhalf)
          coslatj (j)= coslatj(jhalf)
       ENDIF 
    END DO
    ...
    DO jb = 1, jbMax
       DO ib = 1, ibMaxPerJB(jb)
          ...
          longit  (ib,jb)=(i-1)*pai*2./iMaxPerJ(j)
          sinlon  (ib,jb)=SIN(longit(ib,jb))
          coslon  (ib,jb)=COS(longit(ib,jb))
          cos2lat (ib,jb)=1./rcl(ib,jb)
!**(JP)** mudei
