...
CONTAINS
  ...
  SUBROUTINE TimeStep (fb1, fa, fb, &
    ...
    DO j = jFirst, jLast
       CALL coszmed(idatec,tod,nfprt,nferr,yrl,lati(:,j),long(:,j),cosz(:,j),iMax)
    END DO
    !$OMP BARRIER
    ...
    IF (.NOT. inirestart) THEN
       ...
       DO
          ...
          CALL grpcomp ( &
               ...
               intcosz         , fgyu1(1,1,jb)   , fgyv1(1,1,jb)   , fgtd1(1,1,jb)   , &
               fgqd1(1,1,jb))
       END DO
       !$OMP BARRIER
       IF (cdhl(jdt)) THEN
