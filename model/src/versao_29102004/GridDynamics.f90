...
CONTAINS
  ...
  SUBROUTINE GrpComp(&
    ...
    ! INTEGER, PARAMETER :: ndsdot = 8  ! sigma dot
    ! INTEGER, PARAMETER :: ndomg  = 7  ! omega
    ! INTEGER, PARAMETER :: ndpwtr = 14 ! precipitable water     
    ! INTEGER, PARAMETER :: nddq   = 51 ! divergence * specific humidity
    ! INTEGER, PARAMETER :: ndvqad = 52 ! vertical moisture advection
    ! INTEGER, PARAMETER :: ndtd   = 64 ! time mean deep soil temperature
    ! INTEGER, PARAMETER :: ndtg   = 65 ! ground/surface cover temperature
    ! INTEGER, PARAMETER :: ndtc   = 66 ! canopy temperature
    !--------------------------------------------------------------------------              
    ...
    ENDIF
    !
    IF(dodia(ndtd).or.dodia(ndtg).or.dodia(ndtc))THEN
       ncount=0
       DO i=1,ibLim
          gdt(i,1)=gtsea (i,jb)
          gdt(i,2)=gtsea (i,jb)
       ...
       END DO
    END IF
    IF(dodia(ndtd))CALL updia(gdt(1,3),ndtd,latco)
    IF(dodia(ndtg))CALL updia(gdt(1,2),ndtg,latco)
    IF(dodia(ndtc))CALL updia(gdt(1,1),ndtc,latco)
    !
    ...
    IF(IsGridHistoryOn())THEN
       DO k=1,kMax
          ...
          END DO
       END DO
       IF(dogrh(ngt,  latco)) CALL StoreGridHistory (gtd,             ngt,  latco)
       IF(dogrh(ngu,  latco)) CALL StoreGridHistory (gu,              ngu,  latco, sqrt(rcl))
       IF(dogrh(ngv,  latco)) CALL StoreGridHistory (gv,              ngv,  latco, sqrt(rcl))
       IF(dogrh(ngq,  latco)) CALL StoreGridHistory (gq,              ngq,  latco)
       IF(dogrh(ngts, latco)) CALL StoreGridHistory (gtd(:,1),        ngts, latco)
       IF(dogrh(ngus, latco)) CALL StoreGridHistory (gu(:,1),         ngus, latco, sqrt(rcl))
       IF(dogrh(ngvs, latco)) CALL StoreGridHistory (gv(:,1),         ngvs, latco, sqrt(rcl))
       IF(dogrh(ngqs, latco)) CALL StoreGridHistory (gq(:,1),         ngqs, latco)
       IF(dogrh(ngsn, latco)) CALL StoreGridHistory (sheleg(:,latco), ngsn, latco)
       IF(dogrh(ngz0, latco)) CALL StoreGridHistory (zorl(:,latco),   ngz0, latco)
       ncount=0
       DO i=1,ibLim
       ...
       IF(dogrh(ngtc,    latco)) CALL StoreGridHistory (gtd(:,1), ngtc,    latco)
       IF(dogrh(ngtg,    latco)) CALL StoreGridHistory (gtd(:,2), ngtg,    latco)
       IF(dogrh(ngtd,    latco)) CALL StoreGridHistory (gtd(:,3), ngtd,    latco)
       IF(dogrh(ngw1,    latco)) CALL StoreGridHistory (gtd(:,4), ngw1,    latco)
       IF(dogrh(ngw2,    latco)) CALL StoreGridHistory (gtd(:,5), ngw2,    latco)
       IF(dogrh(ngw3,    latco)) CALL StoreGridHistory (gtd(:,6), ngw3,    latco)
       IF(dogrh(ngcapac, latco)) CALL StoreGridHistory (gtd(:,7), ngcapac, latco, 1000.0)
       IF(dogrh(ngcapag, latco)) CALL StoreGridHistory (gtd(:,8), ngcapag, latco, 1000.0)
       IF(dogrh(ngty,    latco)) CALL StoreGridHistory (gtd(:,9), ngty,    latco)
    END IF
    gtd=0.0
    !
    ...
    IF(dodia(nddq)  ) CALL updia(psint,nddq,latco)
    IF(dodia(ndvqad)) CALL updia(divint,ndvqad,latco)
    IF(dodia(ndomg) ) CALL updia(omg,ndomg,latco)
    !
    !
    ...
    !
    IF (IsGridHistoryOn()) THEN
       IF(dogrh(ngpsf, latco)) CALL StoreGridHistory(gplam, ngpsf, latco)
    END IF
    !     
    !     sigma gke computed only at interior interfaces.
    !
    !
    IF(dodia(ndsdot))CALL updia(dot,ndsdot,latco)
    ...
    IF(isimp.ne.'YES ')THEN  
       ...
       IF(dodia(ndpwtr))CALL updia(dot,ndpwtr,latco)
       !
    ...
    !
    !
    IF(dodia(ndpsm))CALL updia(exp(glnp)  ,ndpsm,latco)
    !
