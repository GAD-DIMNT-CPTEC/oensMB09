...
CONTAINS
  ...
  SUBROUTINE Nlnmi(nlnminit,diabatic,slagr,fName, & 
    ...
    ALLOCATE(RealBuffer(kMax*(2*kMax+3)), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
    ...
    CALL SetMods(gh)
    ALLOCATE (qgenp_si(2*mnMax_si,kmax))
    ALLOCATE (qgenp(2*mymnMax,modsloc))
    ...
    ALLOCATE(all_jg(2*mMax,mods), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
       ...
       STOP
    END IF
    ALLOCATE(all_nas(2*mMax,mods), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
       ...
       STOP
    END IF
    ALLOCATE(one_g(3*(mMax+1)*(mMax+1)), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
       ...
       STOP
    END IF
    ALLOCATE(one_per(3*(mMax+1)*(mMax+1)), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
    ...
    ALLOCATE(indper(2*mymMax,modsloc), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
       ...
       STOP
    END IF
    ALLOCATE(indg(2*mymMax,modsloc), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
       ...
       STOP
    END IF
    ALLOCATE(jg(2*mymMax,modsloc), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
       ...
       STOP
    END IF
    ALLOCATE(nas(2*mymMax,modsloc), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
    ...
    ALLOCATE(per(tot_per), STAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i10)") ierr
    ...
    DO iter=1,niter
       ...
       !  --------------------------------------
       CALL TimeStep(fb1,fa,fb,slagr,nlnminit,.FALSE.,.FALSE.,dt,0,ifday, tod, idatec,0)
       !
