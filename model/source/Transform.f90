...
MODULE Transform
  ...
  ! Fourier Fields
  !
  ! Array Four(dvdlj, dip1), using (nVertGrid*myJMax, mMax+1)
  ...
  REAL(KIND=r8), ALLOCATABLE :: FourSp(:,:)
  REAL(KIND=r8), ALLOCATABLE :: SpFour(:,:)
...
CONTAINS
  ...
  SUBROUTINE InitTransform()
    ...
    p%nLong = iMaxPerJ(myfirstlat)
    p%firstLat = 1
    DO j = myfirstlat+1,mylastlat
       IF (iMaxPerJ(j) /= p%nLong) THEN
          p%lastLat = j - myfirstlat
          iBlockFFT = iBlockFFT + 1
          p => BlockFFT(iBlockFFT)
          p%nLong = iMaxPerJ(j)
          p%firstLat = j+1-myfirstlat
       END IF
    END DO
    IF (iBlockFFT /= nBlockFFT) THEN
       ...
       STOP h
    ELSE
       p%lastLat = mylastlat+1-myfirstlat
    END IF
  ...
  SUBROUTINE CreateSpecToGrid(nFullSpec, nSurfSpec, nFullGrid, nSurfGrid)
    ...
    ALLOCATE (FourSp(nVertSpec*myjMax*2*MMaxlocal,0:maxNodes-1))
    ALLOCATE (SpFour(nVertSpec*jMaxlocal*2*myMMax,0:maxNodes-1))
    IF (.NOT.ALLOCATED(requests)) THEN
       ALLOCATE(requests(0:maxnodes-1))
       ALLOCATE(requestr(0:maxnodes-1))
  ...
  SUBROUTINE DoSpecToGrid()
    ...
    IF (usedSpecFields /= nSpecFields) THEN
       WRITE(nfprt,*) ' used, nspec ',usedSpecFields, nSpecFields 
    ...
    ELSE IF (usedDelLamFields /= nGridFields) THEN
       WRITE(nfprt,"(a, ' not all gauss and dellam fields were deposited')") h
       STOP h
    END IF
...
!TO       CALL InvFFTTrans (Four(FFTfirst ,1), dvdlj, dip1, p%nLong, FFTSize, &
!TO            p%Trigs, p%nTrigs, p%Factors, p%nFactors)
    END DO
    !$OMP BARRIER
  ...
  SUBROUTINE SpecToFour()
    ...
    CALL ThreadDecomp(1,myjmax,jfirst,jlast,"SpecToFour")
    !$OMP SINGLE
    mGlob = 0
    !$OMP END SINGLE
    DO
       ...
       DO jn = jMinPerM(m), jMaxHalf
          ...
          kn = nodeHasJ(jn)
          ks = nodeHasJ(js)
          inin = (jn-firstlatinproc(kn)+nlatsinproc(kn)*2*(lm-1))*nVertSpec
          inin1 = inin + nlatsinproc(kn)*nVertSpec
          inis = (js-firstlatinproc(ks)+nlatsinproc(ks)*2*(lm-1))*nVertSpec
          inis1 = inis + nlatsinproc(ks)*nVertSpec
          !CDIR NODEP
          DO v = 1, nVertSpec
             SpFour(inin +v,kn) = FoEv(v,          jn) + FoOd(v,          jn)
             SpFour(inin1+v,kn) = FoEv(v+nVertSpec,jn) + FoOd(v+nVertSpec,jn)
             SpFour(inis +v,ks) = FoEv(v,          jn) - FoOd(v,          jn)
             SpFour(inis1+v,ks) = FoEv(v+nVertSpec,jn) - FoOd(v+nVertSpec,jn)
          END DO
       END DO
    END DO
    !$OMP BARRIER
    ...
    DO k=0,MaxNodes-1
       IF(k.ne.myid) THEN
          CALL MPI_IRECV(FourSp(1,k),2*myjmax*nvertspec*MMaxinproc(k), &
               MPI_DOUBLE_PRECISION,k,97,comm,requestr(k),ierr)
       ENDIF
    ENDDO
    DO k=0,MaxNodes-1
       IF(k.ne.myid) THEN
          CALL MPI_ISEND(SpFour(1,k),2*nlatsinproc(k)*nvertspec*mymmax, &
    ...
    mm = 0
    DO m=1,Mmax
       IF (myid.eq.nodeHasM(m)) THEN
          mm = mm+1
          inin = (myjmax*2*(mm-1))*nVertSpec
          inin1 = inin + myjmax*nVertSpec
          DO j=jfirst,jlast
              DO v = 1, nVertSpec
                 Four((j-1)*nVertGrid + v, 2*m-1) = &
                                SpFour((j-1)*nVertSpec+v+inin,myid)
                 Four((j-1)*nVertGrid + v, 2*m  ) = &
                                SpFour((j-1)*nVertSpec+v+inin1,myid)
             END DO
          END DO
       ENDIF
    END DO
       ...
       DO m=1,Mmax
          IF (ks.eq.nodeHasM(m)) THEN
             ...
             DO j=jfirst,jlast
                 DO v = 1, nVertSpec
                    Four((j-1)*nVertGrid + v, 2*m-1) = &
                                   FourSp((j-1)*nVertSpec+v+inin,ks)
                    Four((j-1)*nVertGrid + v, 2*m  ) = &
                                   FourSp((j-1)*nVertSpec+v+inin1,ks)
                END DO
             END DO
          ENDIF
       END DO
    ENDDO
    !$OMP SINGLE
    CALL MPI_WAITALL(maxnodes,requests,stat,ierr)
    !$OMP END SINGLE
          ...
          IF (fieldForDelLam(ig)) THEN
             ...
             DO v = 1, vMax
                DO j = 1, myjMax
                   ...
                   auxRe = consIm * Four(jAux + vBaseFrom, 2*m-1)
                   Four(jAux + vBaseTo, 2*m-1) = consRe * Four(jAux + vBaseFrom, 2*m  )
  ...
  SUBROUTINE FourToSpec()
    ...
    DO 
       ...
       IF (kn.ne.myid) THEN
          !CDIR NODEP
          DO j = MAX(jMinPerM(m),myfirstlat),MIN(mylastlat,jMaxPerM(m))
             DO v = 1, nVertSpec
                jl = j-myfirstlat+1
                FourSp((myjmax*2*(lm-1)+(jl-1))*nVertGrid + v, kn) = &
         ...
         ELSE
          DO j = MAX(jMinPerM(m),myfirstlat),MIN(mylastlat,jMaxPerM(m))
             DO v = 1, nVertSpec
                jl = j-myfirstlat+1
                SpFour((myjmax*2*(lm-1)+(jl-1))*nVertGrid + v, kn) = &
                                               Four((jl-1)*nVertGrid + v, 2*m-1)
                SpFour((myjmax*(2*lm-1)+(jl-1))*nVertGrid + v, kn) = &
                                               Four((jl-1)*nVertGrid + v, 2*m)
             END DO
    ...
    DO k=0,MaxNodes-1
       IF(k.ne.myid) THEN
          CALL MPI_IRECV(SpFour(1,k),2*nlatsinproc(k)*nvertspec*mymmax, &
               MPI_DOUBLE_PRECISION,k,98,comm,requestr(k),ierr)
       ENDIF
    ENDDO
    DO k=0,MaxNodes-1
       IF(k.ne.myid) THEN
          CALL MPI_ISEND(FourSp(1,k),2*myjmax*nvertspec*MMaxinproc(k), &
    ...
    DO
       ...
       DO j = jMinPerM(m), jMaxHalf
          ...
          kn = nodeHasJ(j)
          ks = nodeHasJ(js)
          inin = (nlatsinproc(kn)*2*(lm-1)+j-firstlatinproc(kn))*nVertSpec
          inin1 = inin + nlatsinproc(kn)*nVertSpec
          inis = (nlatsinproc(ks)*2*(lm-1)+js-firstlatinproc(ks))*nVertSpec
          inis1 = inis + nlatsinproc(ks)*nVertSpec
          DO v = 1, nVertSpec
             FoEv(j,v          ) = SpFour(inin+v,kn)  + SpFour(inis+v,ks)
             FoEv(j,v+nVertSpec) = SpFour(inin1+v,kn) + SpFour(inis1+v,ks)
             FoOd(j,v          ) = SpFour(inin+v,kn)  - SpFour(inis+v,ks)
             FoOd(j,v+nVertSpec) = SpFour(inin1+v,kn) - SpFour(inis1+v,ks)
          END DO
       END DO
       DO j = 1, dv
