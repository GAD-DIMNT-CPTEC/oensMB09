...
CONTAINS
  SUBROUTINE SemiLagr(nit , delt, inew, iold)
    !
    ...
    IF (maxnodes.gt.1) THEN
       ...
       CALL Exchange_Fields(fgyum,fgyvm,fgtdm,fgqdm,fgvdlnpm,fgpass_scalars, &
                            nscalars,iold,nrecs_gr,nsends_gr)
       !$OMP END SINGLE
    ENDIF
    !
    ...
    DO
       ...
       ! ----------------------------
       DO k=1,nscalars
          CALL Interpcublin3d(fgpass_scalars(1,1,jb,k,inew), &
               fgpass_scalars(1,1,1,k,iold), .TRUE. , &
               ulondp, ulatdp, usigdp, &
               iloc  , jloc  , kloc  , &
               ibdim ,  1.0_r8  ,.FALSE.,  &
               ip , jn, &
  ...
  SUBROUTINE SemiLagr_2tl(nit , delt, inew, iold, fa)
    !
    ! Trajectory: Determination of the departure point of Lagrangian trajectory
    !
    !
