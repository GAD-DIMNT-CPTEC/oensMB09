...
CONTAINS
  ...
  SUBROUTINE VerticalGroups(givenfouriergroups,nproc_vert)
    ...
    ENDDO
    ALLOCATE(nset(ng))
    ALLOCATE(map_four(ng,0:MAXVAL(npperg_four)-1))
    n = 0
    nset = 0
    DO
       DO i=1,ngroups_four
  ...
  SUBROUTINE SpectralDomainDecomp()
    ...
    ALLOCATE(msPerProc(0:maxNodes_four-1), stat=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i8)") ierr
    ...
    ALLOCATE(msInProc(mMaxLocal,0:maxNodes_four-1), stat=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i8)") ierr
    ...
    ALLOCATE(nodeHasM(mMax,ngroups_four), stat=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i8)") ierr
    ...
    ALLOCATE(lm2m(myMMax), stat=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i8)") ierr
    ...
    ALLOCATE(mnsPerProc(0:maxNodes_four-1), stat=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i8)") ierr
    ...
    ALLOCATE(mnsExtPerProc(0:maxNodes_four-1), stat=ierr)
    IF (ierr /= 0) THEN
       WRITE(c0,"(i8)") ierr
       CALL FatalError(h//" allocate mnsPerProc returned stat="//TRIM(ADJUSTL(c0)))
    END IF
...
!   END IF
  ...
  !
    ...
    ALLOCATE (mnsinProc_si(0:maxnodes-1))
    ALLOCATE (mnProcmap_si(mnmax))
...
!   mylastmn_si = myfirstmn_si + MNMax_si - 1
    ...
    ALLOCATE (mynMap_si(MNMax_si))
    ALLOCATE (mymnMap_si(MNMax_si))
    ...
    ALLOCATE (maps_si(0:maxnodes-1))
    ALLOCATE (mapr_si(0:maxnodes-1))
    ALLOCATE (sends(maxnodes))
    ...
    ALLOCATE (mysends_si(nsends_si))
    ALLOCATE (inibs_si(1:nsends_si+1))
  ...
  SUBROUTINE DumpSpecDecomp(maxNodes)
    ...
    DO peStart = 0, maxNodes-1, 10
       peEnd = MIN(peStart+9, maxNodes-1)
       line = " msPerProc(    :    )="
  ...
  SUBROUTINE GridDecomposition(ibmax,jbmax,jbmax_ext,nproc,myid,vmax_est)
    ...
    IF (SL_twotime_scheme) THEN
       jovlap = (delt*vmax_est*imax)/40024000._r8 + 2
     ELSE
       jovlap = (2._r8*delt*vmax_est*imax)/40024000._r8 + 2
    ENDIF
    IF (.not.slagr) jovlap = 0
...
!   if (myid.eq.0) then
!   do iproc=0,nproc-1
!      write(*,*) 'proc ',iproc,' first and last lat ',firstlat(iproc),lastlat(iproc)
!      do j=firstlat(iproc),lastlat(iproc)
!         j1 =j-firstlatingroup(procingroup(iproc))+1
!          write(*,*) 'line ',j,' from ',firstlon(j1,iproc),' to ',lastlon(j1,iproc)
!      enddo
!   enddo
!   endif
    ...
    IF (slagr) THEN
       ...
       DO jb=j1,j2
          ...
          i1 = NINT(ifirst*imaxperj(j)) - iovlap(j)
          i2 = NINT(ilast*imaxperj(j)) + iovlap(j)
