...
161 FORMAT(' ',A40,2X,A4,2X,I8,3X,I4,4X,I3)    
  ...
  SUBROUTINE Readms(mPerLat,fname,jmax)
    ...
    OPEN (UNIT=nfwaves, FILE=TRIM(fname))
    READ (nfwaves,*) jsize
    IF (jsize.NE.jmax) THEN
       WRITE(UNIT=nfprt,FMT="('**(ERROR)** Read file ',a,' sizes not match  ',i8)") &
            TRIM(fname), jsize
       STOP "**(ERROR)**"
    END IF
    READ (nfwaves,*)(mperlat(j),j=1,jmax)
    CLOSE (UNIT=nfwaves)
  END SUBROUTINE Readms

  SUBROUTINE Readgauss(CosGaussColat ,GaussWeights,fname,jmax2)
    CHARACTER(LEN=*), INTENT(IN ) :: fname
    INTEGER,          INTENT(IN ) :: jmax2
    REAL(KIND=r8),    INTENT(OUT) :: CosGaussColat (jmax2)
    REAL(KIND=r8),    INTENT(OUT) :: GaussWeights(jmax2)
    INTEGER  :: jsize

    OPEN (UNIT=nfgauss, FILE=TRIM(fname), FORM='unformatted')
    READ (nfgauss) jsize
    IF (jsize.NE.jmax2) THEN
       WRITE(UNIT=nfprt,FMT="('**(ERROR)** Read file ',a,' sizes not match  ',i8)") &
            TRIM(fname), jsize
       STOP "**(ERROR)**"
    END IF
    READ (nfgauss) CosGaussColat ,GaussWeights
    CLOSE (UNIT=nfgauss)
  ...
  SUBROUTINE ReadLandSeaMask2_4(fname, lsmk)
    ...
    OPEN (UNIT=nfsst, FILE=TRIM(fname), FORM='unformatted', ACCESS='sequential',&
    ...
    IF (ierr /= 0) THEN
       ...
       STOP "**(ERROR)**"
    END IF
    READ (UNIT=nfsst, IOSTAT=ierr) lsmk
    IF (ierr /= 0) THEN
  ...
  SUBROUTINE ReadLandSeaMask2_8(fname, lsmk)
    ...
    IF (ierr /= 0) THEN
       ...
       STOP "**(ERROR)**"
    END IF
    READ (UNIT=nfsst, IOSTAT=ierr) aux
    IF (ierr /= 0) THEN
       WRITE(UNIT=nfprt,FMT="('**(ERROR)** Read file ',a,' returned iostat=',i4)") &
            TRIM(fname), ierr
       STOP "**(ERROR)**"
...
336 FORMAT(' FOR IFSST=',I5,' SSTLAG MUST BE SET NONNEGATIVE.  NOT ',G12.5)  
  ...
  SUBROUTINE ReadNFTGZ4(n,field1,field2,field3,field4)
    ...
    READ  (UNIT=n) raux1,raux2,raux3,raux4
