...
MODULE FieldsPhysics
  ...
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xvisb ! Downward Surface shortwave fluxe visible beam (cloudy)  (W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xvisd ! Downward Surface shortwave fluxe visible diffuse (cloudy) (W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xnirb ! Downward Surface shortwave fluxe Near-IR beam (cloudy)  (W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xnird ! Downward Surface shortwave fluxe Near-IR diffuse (cloudy)  (W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xswtop! Shortwave upward at top  or  shortwave upward at top (clear)(W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xvisbc! Downward Surface shortwave fluxe visible beam (clear) (W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xvisdc! Downward Surface shortwave fluxe visible diffuse (clear)(W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xnirbc! Downward Surface shortwave fluxe Near-IR beam (clear)(W/m^2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: xnirdc! Downward Surface shortwave fluxe Near-IR diffuse (clear)(W/m^2)

  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: yvisb ! Downward Surface shortwave fluxe visible beam (cloudy)  (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: yvisd ! Downward Surface shortwave fluxe visible diffuse (cloudy) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: ynirb ! Downward Surface shortwave fluxe Near-IR beam (cloudy) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: ynird ! Downward Surface shortwave fluxe Near-IR diffuse (cloudy) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: yswtop! Solar input at top of atmosphere (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: yvisbc! Downward Surface shortwave fluxe visible beam (clear) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: yvisdc! Downward Surface shortwave fluxe visible diffuse (clear) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: ynirbc! Downward Surface shortwave fluxe Near-IR beam (clear) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: ynirdc! Downward Surface shortwave fluxe Near-IR diffuse (clear) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: cldsav! Cloud cover
  ...
  ! radiative convective cloud minimum base layer index
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:,:) :: htrc! Heating rate due to shortwave (clear) (K/s)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:,:) :: clrc! Cooling rate due to longwave  (clear) (K/s)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rvisbc! Visible beam clear sky (Refer to downward surface
  ! shortwave fluxe) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rvisdc! Visible diffuse cloudy skies (refer to downward surface
  ! fluxes)  (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rnirbc! Visible beam clear sky (Refer to downward surface
  ! shortwave fluxe) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rnirdc! Near-IR diffuse clear skies (Refer to downward surface
  ! shortwave fluxe)(W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: dlwclr! Downward longwave (clear) At the bottom (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: uswtpc! Shortwave upward at top (clear) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rsclr ! Net longwave at bottom (clear) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: ultclr! Longwave upward at top (clear)   (W/m2)

  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: avisb ! Visible beam surface albedo
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: avisd ! Visible diffuse surface albedo
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: anirb ! Near-ir beam surface albedo
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: anird ! Near-ir diffuse surface albedo
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: dswtop! Solar input at top of atmosphere(W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rs    ! Net surface ir radiation in (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: ulwtop! Long wave flux at top of atmosphere in w/m**2
  ...
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:,:) :: htr ! Heating rate due to shrt wave radiation in (K/sec )
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:,:) :: clr ! Cooling rate due to long wave radiation in (K/sec )
  INTEGER(KIND=i8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: imask ! vegetation mask
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rvisb ! Visible beam cloudy skies(refer to downward surface fluxes) (W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rvisd ! Visible diffuse cloudy skies(refer to downward surface fluxes)(W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rnirb ! Near-ir beam  cloudy skies(refer to downward surface fluxes)(W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: rnird ! Near-ir diffuse cloudy skies(refer to downward surface fluxes)(W/m2)
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:) :: dlwbot! Downward longwave radiation at the bottom in (W/m2)
  ...
  REAL(KIND=r8), PUBLIC, ALLOCATABLE, DIMENSION(:,:,:,:) :: cloud
...
CONTAINS
  ...
  SUBROUTINE InitFieldsPhyscs(ibMax, kMax, jbMax,iMax_in,jMax_in)
    ...
    htrc  =0.0_r8
    ALLOCATE(clrc  (ibMax,kMax,jbMax))
    clrc  =0.0_r8
    ...
    evap  =0.0_r8
    ALLOCATE(cloud(ibMax,kMax,8,jbMax))
    cloud=0.0_r8
  ...
  SUBROUTINE InitVariancia(igwd,nfvar,fNameOrgvar)
    ...
    var_in  =0.0_r8
    OPEN(UNIT=nfvar, FILE=TRIM(fNameOrgvar),FORM='unformatted',ACCESS='sequential',&
    ...
    IF(TRIM(isimp).NE.'YES') THEN
       IF(nfcnv0.NE.0) THEN
          ...
          READ(UNIT=nfcnv0) convc,convt,convb,prcp1,prcp2,prcp3, &
               prcpt,toplv,botlv
          IF(ifday.GT.0.OR.tod.GT.0.0_r8)READ(UNIT=nfcnv0)rvisd,rvisb,rnird, &
               rnirb,rvisdc,rvisbc,rnirdc,rnirbc,dswtop
       ...
       OPEN(UNIT=nfsibt, FILE=TRIM(fNameSibmsk),FORM='unformatted',ACCESS='sequential',&
            ACTION='read',STATUS='old', IOSTAT=ierr)
       IF (ierr /= 0) THEN
          WRITE(UNIT=nfprt,FMT="('**(ERROR)** Open file ',a,' returned iostat=',i4)") &
       ...
       OPEN(UNIT=nftgz0, FILE=TRIM(fNameTg3zrl),FORM='unformatted',ACCESS='sequential',&
            ACTION='read',STATUS='old', IOSTAT=ierr)
       IF (ierr /= 0) THEN
          WRITE(UNIT=nfprt,FMT="('**(ERROR)** Open file ',a,' returned iostat=',i4)") &
...
555 FORMAT(' CLOUD PROGNOSTIC DATA READ IN. AT FORECAST DAY', &
  ...
  SUBROUTINE read_gl_sm_bc(iMax           , &!   IN
    ...
    PRINT*,'----------------------------------------------'
    PRINT*,'Opening GL soil file=',TRIM(fNameSoilType)
    FracOcc=0.0_r8
    glsm=0.0_r8
    SoilType=0.0_r8
    ...
    !
    PRINT*,'----------------------------------------------'
    PRINT*,'Opening GL veg file=',TRIM(fNameVegType)
    ...
    !
    PRINT*,'----------------------------------------------'
    PRINT*,'Opening GL_SM file=',TRIM(fNameSoilMoist)
    ...
    ELSE
       ...
       STOP 'read_gl_sm: unknown record type'
  ...
  SUBROUTINE restartphyscs (jbMax,ifday,tod,idate ,idatec, &
    ...
    IF(TRIM(isimp).NE.'YES') THEN
       ...
       WRITE(UNIT=nfcnv1) convc,convt,convb,prcp1,prcp2,prcp3, &
            prcpt,toplv,botlv
       WRITE(UNIT=nfcnv1) rvisd,rvisb,rnird,rnirb, &
            rvisdc,rvisbc,rnirdc,rnirbc,dswtop
       !$OMP END SINGLE
    END IF
