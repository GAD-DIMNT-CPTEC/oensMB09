...
contains
  ...
  ! !INTERFACE:
  !
  subroutine bilinear_interp_init (gridDesci,gridDesco,&
                                   rlat,rlon,          &
                                   w11,w12,w21,w22,    &
                                   n11,n12,n21,n22     &
                                   )
  ...
  !-------------------------------------------------------------------------!
  !BOC
     character(len=64), parameter :: myname_=trim(myname)//' :: bilinear_interp_init( )'
     ...
     if(gridDesco(1).ge.0) then
        call compute_earth_coord(gridDesco, fill, .true., xpts, ypts, rlon, rlat, nv)
     endif
     ...
     enddo
  end subroutine bilinear_interp_init
  ...
  ! !INTERFACE:
  !
  subroutine bilinear_interp( GridDesco, lato, lono, &
                              ibitmap, iField, UDef, &
                              w11, w12, w21, w22,    &
                              n11, n12, n21, n22,    &
                              obitmap, oField,       &
  ...
  !
    ...
    real,    dimension(:), intent(in   ) :: GridDesco          ! Grid description of output field
    real,    dimension(:), intent(in   ) :: lato               ! latitude of output field
    real,    dimension(:), intent(in   ) :: lono               ! longitude of output field
