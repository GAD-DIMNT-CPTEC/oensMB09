      ...
      membn=1
            while [ $membn -le 7 ]; do # VIRA OS MEMBROS 01 a 07
                  ...
                  while [ $t -le $DFSCSKILL ]; do
                        ...
                        if [ $npc -lt 3 ]; then
                           PREFXN=$membn${NP[$npc]}
                           ARQINGRB=$(find ${DIRARQ}/pos/dataout/${CASE}/$yyyyd$mmd$ddd$hhd/ -name GPOS$membn${NP[$npc]}$datefct${LABELI}P.${icnfct}.${CASE}.grb)
                           echo $ARQINGRB
                           ...
                           PREFXN=${PREFX}
                           membn=999999
                           ARQINGRB=$(find ${DIRARQ}/pos/dataout/${CASE}/$yyyyd$mmd$ddd$hhd/ -name GPOS${PREFX}$datefct${LABELI}P.${icnfct}.${CASE}.grb)
                           CTLNAME=$(echo $ARQINGRB | sed -e s#grb#ctl#g | sed -e s#./ctl/#""#g)
                           IDXNAME=$(echo $ARQINGRB | sed -e s#grb#idx#g | sed -e s#./ctl/#""#g)
                           echo $ARQINGRB
                           ln -sf $CTLNAME ./ctl/GPOS${PREFXN}$datefct${LABELI}P.${icnfct}.${CASE}.ctl
