...
SCRIPTNAME=setperreco.${PREFIC}${RESOL}${NIVEL}.${LABELI}
...
export SECOND='     '
export it
echo ". ${OPERM}/run/runrecfct.${MACHINE} ${COMPILE} ${TRC} ${LV} FCT\${NUM}R \${LABELI} \${i}  \${NUM}R"
. ${OPERM}/run/runrecfct.${MACHINE} ${COMPILE} ${TRC} ${LV} FCT\${NUM}R \${LABELI} \${i}  \${NUM}R
let it=${it}+1
export it
done
export it=2
export FIRST=\`cat ${OPERM}/run/recfct.\${NUM}R.\${LABELI}\${i}.JOBID\`
echo "(\${it}) lastrecfct => \${FIRST}" > ${OPERM}/run/TESTEperreco.out
#export FIRST='     '
export SECOND='     '
cd ${OPERM}/run
. ${OPERM}/run/runeofs_hn.${MACHINE} ${COMPILE} ${TRC} ${LV} ${HUMID} \${NUM} \${LABELI} \${NFDAYS} ${NUMPERT}
export FIRST=\${JID}
echo "(\${it}) eofs_hn => \${FIRST}" >> ${OPERM}/run/TESTEperreco.out
. ${OPERM}/run/runeofs_hs.${MACHINE} ${COMPILE} ${TRC} ${LV} ${HUMID} \${NUM} \${LABELI} \${NFDAYS} ${NUMPERT}
export FIRST=\${JID}
echo "(\${it}) eofs_hs => \${FIRST}" >> ${OPERM}/run/TESTEperreco.out
. ${OPERM}/run/runeofs_sas.${MACHINE} ${COMPILE} ${TRC} ${LV} ${HUMID} \${NUM} \${LABELI} \${NFDAYS} ${NUMPERT}
export FIRST=\${JID}
echo "(\${it}) eofs_san => \${FIRST}" >> ${OPERM}/run/TESTEperreco.out
. ${OPERM}/run/runeofs_san.${MACHINE} ${COMPILE} ${TRC} ${LV} ${HUMID} \${NUM} \${LABELI} \${NFDAYS} ${NUMPERT}
export FIRST=\${JID}
echo "(\${it}) eofs_sas => \${FIRST}" >> ${OPERM}/run/TESTEperreco.out
. ${OPERM}/run/runeofs_tr.${MACHINE} ${COMPILE} ${TRC} ${LV} ${HUMID} \${NUM} \${LABELI} \${NFDAYS} ${NUMPERT}
echo \${JID} > eofs_tr.${NUM}${PT}.${LABELI}.JOBID
echo "(\${it}) eofs_tr => \`cat eofs_tr.${NUM}${PT}.${LABELI}.JOBID\`" >> ${OPERM}/run/TESTEperreco.out
let it=${it}+1
export it
#
...
qsub -W depend=afterok:`cat model.${NUM}${PT}.${LABELI}.JOBID` ${OPERM}/run/${SCRIPTNAME} | tee perreco.${NUM}${PT}.${LABELI}.JOBID
#qsub ${OPERM}/run/${SCRIPTNAME} | tee perreco.${NUM}${PT}.${LABELI}.JOBID
#
