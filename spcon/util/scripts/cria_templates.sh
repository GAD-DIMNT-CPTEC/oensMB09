...
#
# !INTERFACE:
#      ./cria_templates.sh <opcao1> <opcao2> <opcao3> <opcao4> 
#
# !INPUT PARAMETERS:
...
#            
#            <opcao4> npert     -> número de perturbações 
#
#  Uso/Exemplos: ./cria_templates.sh 2013010100 126 28 7
#                (cria os arquivos ctls para todas as variáveis e regiões e para 7 membros
#                na resolução TQ0126L028 para a data 2013010100)
#                ./cria_templates.sh 2013010100 213 42 7 
#                (cria os arquivos ctls para todas as variáveis e regiões e para 7 membros
#                na resolução TQ0126L028 para a data 2013010100)
# 
# !REVISION HISTORY:
...
inctime=${SUBMIT_HOME}/oensMB09_aval/util/inctime/inctime
gribmap=/opt/grads/2.1.a1/bin/gribmap
...
template() {
  ...
  echo "${fct} ${dataanl} ${datafct} ${datafmt}"

  if [ ${1} -eq 0 ]
  then

    echo "Análise"
cat <<EOF > template_${fct}.ctl
dset ^%e/GPOS%e${dataanl}${dataanl}P.icn.TQ0213L042.grb
*
index ^template_${fct}.idx
*
undef 9.999E+20
*
title PRESSURE HISTORY    PTEC AGCM REVIS 1.0 2000  T021342   COLD
*
dtype grib   255
*
...
endvars
  ...
  ${gribmap} -v -e -i template_${fct}.ctl
...
EOF
    ...
    echo "Previsão ${1}"
cat <<EOF > template_${fct}.ctl
dset ^%e/GPOS%e${dataanl}%y4%m2%d2%h2P.fct.TQ0213L042.grb
*
index ^template_${fct}.idx
*
undef 9.999E+20
...
do
  ...
  datafct=$(${inctime} ${dataanl} +${fct}hr %y4%m2%d2%h2)

  fmtdata ${datafct}

  template ${fct} ${dataanl} ${datafct} 
