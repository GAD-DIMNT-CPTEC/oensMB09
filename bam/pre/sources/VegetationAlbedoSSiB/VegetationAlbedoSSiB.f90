...
PROGRAM VegetationAlbedoSSiB
   ...
   NAMELIST /InputDim/ DirInp, DirOut, DirBCs

   OPEN (UNIT=nfinp, FILE=TRIM(DirInp)//"/"//NameNML, &
...
SUBROUTINE VegMorphoPhysioSSiBData ()
   ...
   OPEN (UNIT=nfvgi, FILE=TRIM(DirInp)//"/"//DirBCs//VegetationBCs, &
         FORM='FORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirInp)//"/"//DirBCs//VegetationBCs, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   OPEN (UNIT=nfvgo, FILE=TRIM(DirOut)//"/"//VegetationOut, &
         FORM='UNFORMATTED', ACCESS='SEQUENTIAL', ACTION='WRITE', &
         STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//"/"//VegetationOut, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
...
SUBROUTINE AlbedoSSiBData ()
   ...
   OPEN (UNIT=nfali, FILE=TRIM(DirBCs)//"/"//AlbedoBCs, &
         FORM='FORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirBCs)//"/"//AlbedoBCs, &
            ' returned IOStat = ', ios 
      STOP  ' ** (Error) **'
   END IF
   DO itp=1,12
   ...
   OPEN (UNIT=nfalo, FILE=TRIM(DirOut)//"/"//AlbedoOut, &
         FORM='UNFORMATTED', ACCESS='SEQUENTIAL', ACTION='WRITE', &
         STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//"/"//AlbedoOut, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
