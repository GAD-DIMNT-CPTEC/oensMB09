PROGRAM SSTWeekly
   ...
   USE InputParameters, ONLY : r4, r8, Undef, &
                               ...
                               DateICn, Preffix, Suffix, DirInp, DirOut, &
                               InitInputParameters
   ...
   OPEN (UNIT=nfsti, FILE=TRIM(DirOut)//'/'//VarName//'.'//DateICn(1:8), &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecIn, ACTION='READ', &
         STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file: ', &
              TRIM(DirOut)//'/'//VarName//'.'//DateICn(1:8), &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   OPEN (UNIT=nfsto, FILE=TRIM(DirOut)//'/'//VarName//DateICn(1:8)//nLats, &
...
SUBROUTINE SSTClimatological ()
   ...
   OPEN (UNIT=nfclm, FILE=TRIM(DirClmSST)//'/'//FileClmSST, &
         FORM='FORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
