...
PROGRAM VegetationMask
   ...
   OPEN (UNIT=nfcvm, FILE=TRIM(DirOut)//"/"//VarName//'.dat', &
   ...
   OPEN (UNIT=nfvgm, FILE=TRIM(DirOut)//"/"//VarNameVeg//nLats, &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, ACTION='WRITE', &
         STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//"/"//VarNameVeg//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   IF (GrADS) THEN
      ...
      OPEN (UNIT=nfout, FILE=TRIM(DirOut)//"/"//VarNameVeg//nLats, &
      ...
      OPEN (UNIT=nfctl, FILE=TRIM(DirOut)//"/"//VarNameVeg//nLats//'.ctl', &
            FORM='FORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//"/"//VarNameVeg//nLats//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
