...
MODULE InputParameters
 ...
 use nemsio_gfs
  ...
  CHARACTER (LEN=1024), PUBLIC :: DirInp, DirOut, DirTop, &
...
SUBROUTINE InitParameters (itertrc)
  ...
  NAMELIST /ChopNML/ MendInp,KmaxInp,MendOut, KmaxOut, MendMin, MendCut, Iter, SmthPerCut, &
                     ...
                     DirInp, DirOut, givenfouriergroups, &
...
!   WRITE (UNIT=nfprt, FMT='(2A)')     '  prefix     = ', TRIM(prefix)
  ...
  DGDInp=TRIM(DirInp)   ! //'pre/datain/ '
  DirInp=TRIM(DirInp)   ! //'model/datain/ '
  DirOut=TRIM(DirInp)   ! //'model/datain/ '
  DirTop=TRIM(DirInp)   ! //'pre/dataTop/ '
!  DirTop=TRIM(DirMain) ! //'pre/dataout/ '
! DirSig=TRIM(DirHome)  ! //'sources/Chopping/ '
  DirSig=TRIM(DirInp)   ! //'pre/datain/ '
  DirGrd=TRIM(DirOut)   ! //'pre/dataout/ '
