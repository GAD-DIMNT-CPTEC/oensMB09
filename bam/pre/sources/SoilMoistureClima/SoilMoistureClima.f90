...
PROGRAM SoilMoistureClima
   ...
   WRITE (UNIT=nfprt, FMT='(A)')    ' DirMain = '//TRIM(DirInp)
   WRITE (UNIT=nfprt, FMT='(A)')    ' DirMain = '//TRIM(DirOut)
   ...
   OPEN (UNIT=nfout, FILE=TRIM(DirOut)//'/'//VarName//'.dat', &
   ...
   IF (GrADS) THEN
      OPEN (UNIT=nfctl, FILE=TRIM(DirOut)//'/'//VarName//'.ctl', &
            FORM='FORMATTED', ACCESS='SEQUENTIAL', ACTION='WRITE', &
            STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//'/'//VarName//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
