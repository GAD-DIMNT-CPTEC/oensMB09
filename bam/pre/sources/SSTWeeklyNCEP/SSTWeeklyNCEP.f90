...
PROGRAM SSTWeeklyNCEP
   ...
   OPEN (UNIT=nfsst, FILE=TRIM(DirInp)//'/'//FileSST//'.'//Date, &
         FORM='UNFORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
      IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirInp)//'/'//FileSST//'.'//Date, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   READ  (UNIT=nfsst) SST
   ...
   OPEN (UNIT=nfout, FILE=TRIM(DirOut)//'/'//VarName//'.'//Date(1:8), &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRec, ACTION='WRITE', &
         STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//'/'//VarName//'.'//Date(1:8), &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   IF (GrADS) THEN
      OPEN (UNIT=nfctl, FILE=TRIM(DirOut)//'/'//VarName//'.'//Date(1:8)//'.ctl', &
            FORM='FORMATTED', ACCESS='SEQUENTIAL', ACTION='WRITE', &
            STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//'/'//VarName//'.'//Date(1:8)//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
