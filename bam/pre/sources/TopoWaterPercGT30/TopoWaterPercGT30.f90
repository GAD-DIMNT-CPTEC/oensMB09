...
PROGRAM TopoWaterPercGT30
   ...
   NAMELIST /InputDim/ GrADS, DirInp, DirOut, DirBcs
   ...
   DO n=1,NumBox
      ...
      OPEN (UNIT=nfclm, FILE=TRIM(DirBcs)//TopoName(n)//'.dat', &
            FORM='UNFORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='READ', STATUS='OLD', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirBcs)//TopoName(n)//'.dat', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
      READ  (UNIT=nfclm) Tp
   ...
   OPEN (UNIT=nftop, FILE=TRIM(DirOut)//'/'//VarNameT//'.dat', &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//'/'//VarNameT//'.dat', &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   DO j=1,Jmax
   ...
   OPEN (UNIT=nfwat, FILE=TRIM(DirOut)//'/'//VarNameW//'.dat', &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//'/'//VarNameW//'.dat', &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF 
   DO j=1,Jmax
   ...
   IF (GrADS) THEN
      ...
      DO ma=1,mr
         ...
         DO mb=1,ms
            ...
            OPEN (UNIT=nftpw, FILE=TRIM(DirOut)//'/'//VarNameG//gd//'.dat', &
                  FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecGad, &
                  ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
            IF (ios /= 0) THEN
               WRITE (UNIT=nferr, FMT='(3A,I4)') &
                     ' ** (Error) ** Open file ', &
                       TRIM(DirOut)//'/'//VarNameG//gd//'.dat', &
                     ' returned IOStat = ', ios
               STOP  ' ** (Error) **'
            END IF
            nr=0
            ...
            dxy=360.0_r4/REAL(Imax,r4)
            OPEN (UNIT=nfctl, FILE=TRIM(DirOut)//'/'//VarNameG//gd//'.ctl', &
                  FORM='FORMATTED', ACCESS='SEQUENTIAL', ACTION='WRITE', &
                  STATUS='REPLACE', IOSTAT=ios)
            IF (ios /= 0) THEN
               WRITE (UNIT=nferr, FMT='(3A,I4)') &
                     ' ** (Error) ** Open file ', &
                       TRIM(DirOut)//'/'//VarNameG//gd//'.ctl', &
                     ' returned IOStat = ', ios
               STOP  ' ** (Error) **'
            END IF
