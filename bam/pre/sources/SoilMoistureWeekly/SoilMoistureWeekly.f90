...
PROGRAM SoilMoistureWeekly
   ...
   USE InputParameters, ONLY: r4, r8, &
                              ...
                              nLats, &
                              VarName, VarNameOut, DirInp, DirOut, GrADS, Linear, &
   ...
   OPEN (UNIT=nfclm, FILE=TRIM(DirOut)//'/'//TRIM(VarName)//'.'//TRIM(Date(1:8)), &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecIn, ACTION='READ', &
         STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//'/'//TRIM(VarName)//'.'//TRIM(Date(1:8)), &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   DO k=1,kdim
   ...
   OPEN (FILE=TRIM(DirOut)//'/'//TRIM(VarNameOut)//'.'//TRIM(Date(1:8))//nLats, &
         UNIT=nfout, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file: ', &
              TRIM(DirOut)//'/'//TRIM(VarNameOut)//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   DO k=1,kdim
   ...
   TimeGrADS(6:8)=MonthChar(Month)
      ...
      OPEN (FILE=TRIM(DirOut)//'/'//TRIM(VarNameOut)//'.'//TRIM(Date(1:8))//nLats//'.ctl', &
            UNIT=nfctl, FORM='FORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//'/'//TRIM(VarNameOut)//'.'//TRIM(Date(1:8))//nLats//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
      WRITE (UNIT=nfctl, FMT='(A)') 'DSET '// &
             TRIM(DirInp)//'/'//TRIM(VarNameOut)//'.'//TRIM(Date(1:8))//nLats
