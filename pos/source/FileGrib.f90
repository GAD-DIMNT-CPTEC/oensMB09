!
!  $Author: pkubota $
!  $Date: 2007/10/15 16:49:48 $
!  $Revision: 1.6 $
!
MODULE FileGrib

   USE Constants, ONLY : r4, r8, postclim,RegIntIn,ENS
...
! 
  ...
  USE RegInterp, ONLY : Idim=>IdimOut, Jdim=>JdimOut
...
!
    ...
    case(4)
      kgds(4)=nint(glat(1)*1000.)		! LA(1) LATITUDE OF ORIGIN
    end select
...
!
  ...
  ELSE 
  IF(yy_anl >= 2000) THEN
  kpds(8) =yy_anl-2000         ! YEAR INCLUDING (CENTURY-1) yy_anl
  ELSE
  kpds(8) =yy_anl-1900         ! YEAR INCLUDING (CENTURY-1) yy_anl
  END IF
...
!
  kpds(13)=11               ! INDICATOR OF FORECAST TIME UNIT
  IF(postclim)THEN
  kpds(14)=0               ! TIME RANGE 1
  ELSE
  kpds(14)=(fct_hours-anl_hours)/6	! TIME RANGE 1
  END IF
...
SUBROUTINE WriteGrbField (var,ndim, kgds, kpds, bfr,l)
  ...
  integer :: ji,i,iret,ibs,nbits,kf, ifincr,ifhr
...
!   if( kpds(13).eq.12 )IFINCR=12
   ...
   kpds(14)= IFHR-1
   kpds(15)= IFHR
   kpds(16)= 4		! accumulando
!   ibs=-3.0		! to change table 1
   ENDIF 

   IF( var=='CSSF' .or. var=='CLSF' .or. var=='ROLE' .or. var=='OCAS' &
    .or. var=='SLDS' ) THEN
   IFHR = kpds(14)
   kpds(14)= IFHR-1
   kpds(15)= IFHR
...
!
   call putgben(51,kf,kpds,kgds,Kens,ibs,nbits,bitmap,bfr4,iret)
...
! Write variable in grib format
!
  call putgbn(51,kf,kpds,kgds,ibs,nbits,bitmap,bfr4,iret) ! control on # bytes
