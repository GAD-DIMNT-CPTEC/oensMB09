!
!  $Author: pkubota $
!  $Date: 2007/10/18 11:28:13 $
!  $Revision: 1.6 $
!
MODULE FileAccess

  USE Constants, ONLY : r4, i4, r8, nferr, nfprt, nffct, nfpos, &
                        nfctl, nfmdf, nfppf, nfdir, nfrfd
  IMPLICIT NONE
  ...
  CHARACTER (LEN=7  ) :: rfd   =' ' ! post processed file name of required fields
  ...
  CHARACTER (LEN=128) :: modfls=' ' ! total name of model output arquive with file names
...
!  labelp=namfct2(in-25:in-16)
  ...
  IF (namfct1(in-9:in-8) /= 'ir') exdp(4:5)=namfct1(in-9:in-8)
  extp(3:5)=namfct2(in-10:in-8)
  labelp=namfct2(in-22:in-13)
