...
!$OMP END PARALLEL
    ...
    CALL getsbc(iMax ,jMax ,avisd_in ,gtsea_in ,soilm_in ,sheleg_in,ifday ,tod  , &
                idate ,idatec,mfalb ,mfsst ,mfslm, &
...
!    CHARACTER(LEN=*), INTENT(IN) :: fName2
    ...
    !**TO portable TX7** CALL GetUnit(nfnmi)
    nfnmi=23
...
!$OMP END PARALLEL
       ...
       DO l=1, kmax
          ...
          END DO
       END DO
       CALL vertic(qdivt,eigg,eiggt,dotpro,-1)
       CALL vertic(qrott,eigg,eiggt,dotpro,-1)
       CALL vertic(qgenp,eigg,eiggt,dotpro,-1)
       CALL primes(qrott,qdivt,qgenp,gh,-1)
       CALL horiz1(qrott,qdivt,qgenp,percut,nfnmi)
       CALL primes(qrott,qdivt,qgenp,gh,+1)
       !
  ...
  SUBROUTINE vertic(f,eigg,eiggt,dotpro,input)
  ...
  SUBROUTINE primes(vord,divd,comd,gh,input)
  ...
  SUBROUTINE horiz1(vord,divd,comd,v,nf)
    REAL, INTENT(INOUT) :: vord(2,mnMax,kmax)
    REAL, INTENT(INOUT) :: divd(2,mnMax,kmax)
    REAL, INTENT(INOUT) :: comd(2,mnMax,kmax)
    REAL, INTENT(IN) :: v
  ...
  SUBROUTINE Getmod(slagr,fName,rsettov)
    ...
    !**TO portable TX7** CALL GetUnit(nfg)
    nfg=24
