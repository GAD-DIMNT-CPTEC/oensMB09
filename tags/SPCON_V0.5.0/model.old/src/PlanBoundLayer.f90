...
9976 FORMAT(' MODEL TOO COARSE FOR MULTILAYER BASE LAYER.'/, &
  ...
  SUBROUTINE gauss0(g,d,ncols,kmax,nmax)
    ...
    REAL,    INTENT(inout  ) :: d(ncols,kmax,nmax-1)
    INTEGER :: i 
    INTEGER :: k
    !cdir unroll=2
    DO k = kmax-1, 1, -1
  ...
  SUBROUTINE ympbl0 ( &
    ...
    REAL,    INTENT(inout  ) :: gmt(ncols,kmax,3)
    REAL,    INTENT(inout  ) :: gmq(ncols,kmax,3)
    REAL,    INTENT(inout  ) :: gmu(ncols,kmax,4)
  ...
  SUBROUTINE Gwdd(psfc, u, v, t, chug, chvg, xdrag, ydrag, &
    ...
    REAL,    INTENT(inout  ) :: xdrag(ncols)
    REAL,    INTENT(inout  ) :: ydrag(ncols)

    REAL,    INTENT(in   ) :: t    (ncols,kmax)
    REAL,    INTENT(in   ) :: u    (ncols,kmax)
    REAL,    INTENT(in   ) :: v    (ncols,kmax)

    REAL,    INTENT(inout  ) :: chug (ncols,kmax)
    REAL,    INTENT(inout  ) :: chvg (ncols,kmax)
