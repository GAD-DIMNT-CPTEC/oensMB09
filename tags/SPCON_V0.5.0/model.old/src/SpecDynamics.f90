...
!CDIR NODEP
  ...
  SUBROUTINE ImplDifu(dt, mnRIFirst, mnRILast)
    ...
    INTEGER :: mn, k
    REAL :: work(2*mnMax)
    REAL, ALLOCATABLE, DIMENSION(:,:) :: holda, holdb, holdc, hold
    !
    ...
    IF (dodia(nDiag_hdidif) .OR. dodia(nDiag_hhedif) .OR. &
        ...
        ALLOCATE (hold (2*mnMax,kMax))
    END IF
    IF (dodia(nDiag_hdidif)) THEN
       ALLOCATE (holda (2*mnMax,kMax))
       holda=qdivp
    END IF
    DO k = 1, kMax
       DO mn = mnRIFirst, mnRILast
          qdivp(mn,k) = qdivp(mn,k)/(1.0+dt*dk*workImplDifu(mn))
       END DO
    END DO
    IF (dodia(nDiag_hdidif)) THEN
       hold=qdivp-holda
    ...
    IF (dodia(nDiag_hvodif)) THEN
       ALLOCATE (holdb (2*mnMax,kMax))
       holdb=qrotp
    END IF
    IF (dodia(nDiag_hmodif)) THEN
       ALLOCATE (holdc (2*mnMax,kMax))
       holdc=qqp
    END IF
    IF (diffuseQ) THEN
       DO k = 1, kMax
          DO mn = mnRIFirst, mnRILast
             qtmpp(mn,k) = (qtmpp(mn,k)+work(mn)*ct(k)*qlnpp(mn))/(1.0+work(mn))
             qrotp(mn,k) = qrotp(mn,k)/(1.0+work(mn))
             qqp(mn,k)=(qqp(mn,k)+work(mn)*cq(k)*qlnpp(mn))/(1.0+work(mn))
          END DO
       END DO
    ELSE
       DO k = 1, kMax
...
!$OMP BARRIER
  ...
  SUBROUTINE bmcm(dt, slagr)
    ...
    ELSE
       DO k=1,kmax-1
          h1(k) = tov(k+1) * ( p1(k) - 1.0 )
       END DO
       h1(kmax)=0.0 
       h2(   1)=0.0 
       DO k=2, kmax
          h2(k) = tov(k-1) * ( 1.0 - p2(k) )
       END DO
    END IF
    DO k=1, kmax
