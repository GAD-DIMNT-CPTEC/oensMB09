...
MODULE GridHistory
  ...
  INTEGER, PUBLIC, PARAMETER :: nGHis_swdbot = 19 ! incident short wave flux
  ...
  INTEGER, ALLOCATABLE :: gptslocalord(:)
...
CONTAINS
  SUBROUTINE InitGridHistFields
    ...
    GHAF(1:ngfld)%FldsName= (/ "SURFACE PRESSURE                        ", &
         ...
         "TEMP AT 2-M FROM SFC LAYER              ", "SPECIFIC HUMID AT 2-M FROM SFC LAYER    ", &
         "ZONAL WIND AT 10-M FROM SFC LAYER       ", "MERIDIONAL WIND AT 10-M FROM SFC LAYER  "/)
  ...
  SUBROUTINE InitGridHistory (del, idate, &
       allghf_in, grhflg_in, nfghds, nfghloc, nfghdr,iMax_in,jMax_in,&
       ibMax_in,jbMax_in,ibPerIJ_in,jbPerIJ_in,kMax_in,ibMaxPerJB_in,iMaxPerJ_in)
    INTEGER,           INTENT(in) :: iMax_in
    INTEGER,           INTENT(in) :: jMax_in
    INTEGER,           INTENT(in) :: ibMax_in
    INTEGER,           INTENT(in) :: jbMax_in
    INTEGER,           INTENT(in) :: ibPerIJ_in(-1:,-1:)
    INTEGER,           INTENT(in) :: jbPerIJ_in(-1:,-1:)
    ...
    INTEGER, ALLOCATABLE          :: jsglobal(:)
    INTEGER, ALLOCATABLE          :: jslocalord(:)
    ...
    OPEN(UNIT=nfghds,FILE=TRIM(fNameGHTable),FORM='unformatted',ACCESS='sequential',&
    ...
    OPEN(UNIT=nfghloc,FILE=TRIM(fNameGHLoc),FORM='unformatted',ACCESS='sequential',&
         ACTION='read',STATUS='old', IOSTAT=ierr)
    IF (ierr /= 0) THEN
       WRITE(UNIT=nfprt,FMT="('**(ERROR)** Open file ',a,' returned iostat=',i4)") &
    ...
    DO n = 1, ngpts
       ...
       j           = jblocmin(1)
       jsglobal(n) = j
       ngptsperproc(nodeHasj(j)) = ngptsperproc(nodeHasj(j)) + 1

       IF (j.ge.myfirstlat.and.j.le.mylastlat) THEN
    ...
    ALLOCATE(jslocalord(ngptslocal))
    ALLOCATE(gptslocalord(ngptslocal))
    ALLOCATE(map(ngptslocal))
    nloc = 0
    DO j=myfirstlat,mylastlat
       DO n=1,ngptslocal
          IF (jslocal(n).eq.j) THEN
             nloc = nloc + 1
             !compacta os pontos no array local
             gptslocalord (nloc) = gptslocal(n)
             jslocalord   (nloc) = j
             map(n)              = nloc
          ENDIF
       ENDDO
    ...
    DO nloc=1,ngptslocal
       n = map(nloc)
       GPt(n)%PtCty  = PtCtyGlobal (gptslocalord(n))
       GPt(n)%ptLon  = PtLonGlobal (gptslocalord(n))
       GPt(n)%ptLat  =-(PtLatGlobal(gptslocalord(n)) - 90.0_r8)*(pi/180.0_r8)! (0-pi)--(N-S)
       GPt(n)%ptCoor = PtCoorGlobal(gptslocalord(n))(1:11)
    ENDDO
    nl = 1
    DO n = 1,ngptslocal
       nloc = map(nl)
       IF (gptslocal(nl).eq.n) THEN
          nl = nl + 1
       ENDIF
       IF (nl.gt.ngptslocal) EXIT
    END DO
    ...
    DO nloc = 1, ngptslocal
       n = map(nloc)
       dlon=100.0_r8
       DO i=1,iMaxPerJ( jslocalord(n))
          dlon(i) = ABS(((pi*((i-1)*360.0_r8/REAL(iMaxPerJ( jslocalord(n) ),r8)))/180.0_r8) &
               &         -((pi*GPt(n)%ptLon)/180.0_r8))
       END DO
       IbLocMin=MINLOC (dlon)
       GPt(n)%ibLoc =  ibPerIJ(  IbLocMin(1),  jslocalord(n)    )
       GPt(n)%jbLoc =  jbPerIJ(  IbLocMin(1),  jslocalord(n)    )
...
2135 FORMAT(' REQUESTED GRID POINT HISTORY FIELD TABLE EMPTY OR', &
  ...
  SUBROUTINE WriteGridHistoryTopo (fgzs,TopoGridH,nfghtop)
    ...
    DO nloc=1,ngptslocal
       n = map(nloc)
       grhloc(n)=fgzs(GPt(n)%ibLoc,GPt(n)%jbLoc)/grav
    END DO

    IF (maxnodes.gt.1) THEN
       CALL Collect_Grid_His(grhloc, grhtop, ngpts, ngptslocal, 0, 1, ngptsperproc)
       IF (myid.eq.0) CALL WrTopoGrdHist  (nfghtop,grhtop)
    ELSE
       CALL WrTopoGrdHist  (nfghtop,grhloc)
    ENDIF
  ...
  SUBROUTINE WriteGridHistory (nfghou, ifday, tod, idate)
    ...
    IF (maxnodes.gt.1) THEN
       CALL Collect_Grid_His(qwork, work, ngpts, ngptslocal, 0, nghsl, ngptsperproc)
