...
MODULE Init   ! Version 0 of Nov 25th, 2001
  ...
  USE Sizes, ONLY : ibMax, jbMax,kMax, &
       jMax, jMaxHalf, jMinPerM, iMaxPerJ, jbminus, jbplus, &
  ...
  USE Options, ONLY: nfprt, nscalars, fNamewaves, jovlap
  USE IOLowLevel, ONLY : Readms
...
CONTAINS
  ...
  SUBROUTINE InitAll (trunc, vert, reducedGrid, linearGrid, mgiven, gaussgiven, &
    ...
    ! define basic MPI partition

    CALL MPIMappings(myid,maxnodes)
    ...
    CALL RegisterOtherSizes(iMaxPerLat, mPerLat, myid, maxnodes, jovlap)
    IF (dumpLocal) THEN
    ...
    CALL InitFields(ibMax, kMax, jbMax, jbminus, jbplus, mymnMax, mymnExtMax, &
                    nscalars)
