...
CONTAINS
  ...
  !
  SUBROUTINE scloutsp(unit, field, levs, fact1, nufr, nuto)
    ...
    REAL(KIND=r8) :: fldaux(2*mymnmax,levs), fout(2*mnmax,levs)
    ...
    ELSE
       CALL Collect_Spec(fldaux, fout, levs, 0)
  ...
  SUBROUTINE scloutgr(unit, field, levs, fact1, nufr, nuto)
    ...
    INTEGER, INTENT(IN) :: nufr
    INTEGER, INTENT(IN) :: nuto
    REAL(KIND=r8) :: fldaux(imax*myjmax,levs), fout(imax*jmax,levs)

    fldaux = fact1 * field

    CALL cnvray(fldaux,imax*myjmax*levs,nufr,nuto)
    ...
    ELSE
       ...
       ELSE
          CALL Collect_Grid_Full(fldaux, fout, levs, 0)
       ENDIF
       IF(myid.eq.0) CALL WriteField(unit, fout)
...
!!$       WRITE (UNIT=nfprt, FMT=*) ' fa  (*mra) = ', f1, ' fb  (*mrb) = ', f2
    ...
    IF (ifsnw /= 0) THEN
       IF (ifsnw == 1) THEN
          OPEN(UNIT=nfsnw, file=TRIM(fNameSnow), FORM='unformatted', ACCESS='sequential',&
               ACTION='read', STATUS='old', IOSTAT=ierr)
          IF (ierr /= 0) THEN
             WRITE(UNIT=nfprt,FMT="('**(ERROR)** Open file ',a,' returned iostat=',i4)") &
                  TRIM(fNameSnow), ierr
    ...
    IF (ifslm /= 0) THEN
       IF (ifslm == 1) THEN
          OPEN(UNIT=nfslm, FILE=TRIM(fNameSoilms),FORM='unformatted', ACCESS='sequential',&
               ACTION='read', STATUS='old', IOSTAT=ierr)
          IF (ierr /= 0) THEN
             WRITE(UNIT=nfprt,FMT="('**(ERROR)** Open file ',a,' returned iostat=',i4)") &
...
1999 FORMAT('ABNORMAL END IN SUBR.GETSBC AT SST   INTERPOLATION')
  ...
  SUBROUTINE gread(n, ifday, tod, idate, idatec, &
    ...
    REAL(KIND=r8),    INTENT(OUT) :: qtmp(2*mymnMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qdiv(2*mymnMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qrot(2*mymnMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qq  (2*mymnMax,kMax)
    ...
    ELSE
       ...
       DO k = 1, kMax
          ...
          DO mm=1,mymmax
             ...
             i2 = 2*mymnmap(mm,m)-1
             DO nn=0,2*(mmax-m)+1
                qdiv(i2+nn,k) = aux(i1+nn,1)
                qrot(i2+nn,k) = aux(i1+nn,2)
             ENDDO
          ENDDO
       ENDDO
...
999 FORMAT(' N,KMAX,KMAXP=',3I4)
  ...
  SUBROUTINE gwrite(n, ifday, tod, idate, idatec, &
    ...
    REAL(KIND=r8),    INTENT(IN) :: qtmp (2*mnMax,kMax)
    REAL(KIND=r8),    INTENT(IN) :: qdiv (2*mnMax,kMax)
    REAL(KIND=r8),    INTENT(IN) :: qrot (2*mnMax,kMax)
    REAL(KIND=r8),    INTENT(IN) :: qq   (2*mnMax,kMax)
    REAL(KIND=r8),    INTENT(IN) :: si(kMax+1)
    REAL(KIND=r8),    INTENT(IN) :: sl(kMax)
    ...
    ELSE
       CALL Collect_Spec(qgzs, aux(:,1), 1, 0)
       IF(myid.eq.0) CALL GWriteField(n, aux(:,1))

       CALL Collect_Spec(qlnp, aux(:,1), 1, 0)
       IF(myid.eq.0) CALL GWriteField(n, aux(:,1))

       CALL Collect_Spec(qtmp, aux, kmax, 0)
       IF(myid.eq.0) CALL GWriteField(n, aux)

       CALL Collect_Spec(qdiv, aux, kmax, 0)
       CALL Collect_Spec(qrot, aux1, kmax, 0)
       IF(myid.eq.0) THEN
          DO k = 1, kMax
       ...
       CALL Collect_Spec(qq, aux, kmax, 0)
       IF(myid.eq.0) CALL GWriteField(n, aux)
    ENDIF
