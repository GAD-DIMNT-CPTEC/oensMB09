...
CONTAINS
  ...
  SUBROUTINE surface_driver(&
       ...
       intg  ,ssib  ,zlwup ,nsx   ,tgeff ,salb  ,topo  , &
       ...
       zorl,tseam ,rshort,SICE2, ustar2 ,qsfc0,tsfc0,qsfcm,&
       tsfcm,qsfc,tsfc,z0,qm0,qm  ,qmm,tm0,tm,tmm,htdisp,temp2m,&
       umes2m,uve10m,vve10m)
    ...
    REAL(KIND=r8), INTENT(IN   ) :: rshort   (nCols)
    ...
    REAL(KIND=r8) :: swrk  (nCols,9)
    REAL(KIND=r8) :: fmom
    REAL(KIND=r8) :: topog (nCols)
    ...
    REAL(KIND=r8) :: swland(nCols)

    REAL(KIND=r8):: tmtx1     (nCols,kmax,3)
    REAL(KIND=r8):: qmtx1     (nCols,kmax,3)
    REAL(KIND=r8):: umtx1     (nCols,kmax,4)
    ...
    IF(schemes==1)THEN
       CALL SSiB_Driver(&
            ...
            nsx                ,tmtx(1:nCols,:,:)    ,qmtx(1:nCols,:,:),umtx (1:nCols,:,:),&
            ...
            qmm(1:nCols)       ,tm0(1:nCols)         ,tmm(1:nCols))
    END IF
    ...
    IF( (kt.NE.0) .OR. (jdt.NE.1) ) THEN
       ...
       ncount=0
       DO i=1,nCols
          IF(ghl_local .AND. dogrh(nGHis_lwubot,latitu))bfrg(i)=stefan*tsurf(i)**4
          ...
          IF(imask(i).GE.1) THEN
             ncount=ncount+1
             IF(ghl_local .AND. dogrh(nGHis_lwubot,latitu))bfrg(i)=zlwup(ncount)
