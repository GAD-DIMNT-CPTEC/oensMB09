...
CONTAINS
  ...
  SUBROUTINE DZtoUV(qdivp, qrotp, qup, qvp, mnRIExtFirst, mnRIExtLast)
    REAL(KIND=r8),    INTENT(IN ) :: qdivp(2*mymnMax,kMax)
    REAL(KIND=r8),    INTENT(IN ) :: qrotp(2*mymnMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qup(2*mymnExtMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qvp(2*mymnExtMax,kMax)
  ...
  SUBROUTINE FiltDiss(dt, vmax, kFirst, kLast, mnRIFirst, mnRILast)
    ...
    REAL(KIND=r8),    INTENT(IN) :: vmax(kMax)
  ...
  SUBROUTINE Gozrim2D(q, qder, mnRIExtFirst, mnRIExtLast)
    REAL(KIND=r8),    INTENT(IN)  :: q(2*mymnMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qder(2*mymnExtMax,kMax)
    INTEGER, INTENT(IN ) :: mnRIExtFirst 
    INTEGER, INTENT(IN ) :: mnRIExtLast
    INTEGER :: mn, k
  ...
  SUBROUTINE InitImplDifu(ct_in, cq_in, dk_in, tk_in)
    ...
    REAL(KIND=r8),    INTENT(IN) :: dk_in
    REAL(KIND=r8),    INTENT(IN) :: tk_in
    INTEGER :: mn, m, n
    INTEGER :: mglob
    !
  ...
  SUBROUTINE ImplDifu(dt, mnRIFirst, mnRILast)
    ...
    IF (dodia(nDiag_hdidif) .OR. dodia(nDiag_hhedif) .OR. &
       ...
       ALLOCATE (hold (2*mymnMax,kMax))
    END IF
    IF (dodia(nDiag_hdidif)) THEN
       ALLOCATE (holda (2*mymnMax,kMax))
       holda=qdivp
    END IF
    DO k = 1, kMax
       DO mn = mnRIFirst, mnRILast
    ...
    IF (dodia(nDiag_hvodif)) THEN
       ALLOCATE (holdb (2*mymnMax,kMax))
       holdb=qrotp
    END IF
    IF (dodia(nDiag_hmodif)) THEN
       ALLOCATE (holdc (2*mymnMax,kMax))
       holdc=qqp
    END IF
    IF (diffuseQ) THEN
       DO k = 1, kMax
    ...
    IF (dodia(nDiag_hhedif)) THEN
       hold=qtmpp-holda
       CALL upspec (hold, nDiag_hhedif)
       DEALLOCATE (holda)
    END IF
    IF (dodia(nDiag_hvodif)) THEN
       hold=qrotp-holdb
       CALL upspec (hold, nDiag_hvodif)
       DEALLOCATE (holdb)
    END IF
    IF (dodia(nDiag_hmodif)) THEN
       hold=qqp-holdc
       CALL upspec (hold, nDiag_hmodif)
       DEALLOCATE (holdc)
    END IF
...
!TO!cdir novector
    ...
    IF (MasCon.and..not.MasCon_ps) THEN
       IF (mnRIFirst == 1 .AND. nodeHasM(1) == myid) THEN
          startMasCon = 2
       END IF
    END IF
    IF (slagr) THEN
  ...
  SUBROUTINE Uvtodz(qup, qvp, qdivt, qrott, mnRIFirst, mnRILast)
    REAL(KIND=r8),    INTENT(IN ) :: qup(2*mymnExtMax,kMax)
    REAL(KIND=r8),    INTENT(IN ) :: qvp(2*mymnExtMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qdivt(2*mymnMax,kMax)
    REAL(KIND=r8),    INTENT(OUT) :: qrott(2*mymnMax,kMax)
    INTEGER, INTENT(IN ) :: mnRIFirst
    INTEGER, INTENT(IN ) :: mnRILast
    INTEGER :: mn, k
