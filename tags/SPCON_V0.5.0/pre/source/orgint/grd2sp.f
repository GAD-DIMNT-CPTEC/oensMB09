      SUBROUTINE GRD2SP(GRD,CFT)
C*
C*    GENERAL PURPOSE TRANSFORMATION OF GLOBAL GRID POINTS DATA
C*                    TO SPHERICAL HARMONICS COEFFICIENTS
C*
C*    INPUT:
...
C*
      INTEGER LAT,I,MN,L,NN,MMAX,MM,LX
C*
C*    RESET SPHERICAL HARMONIC COEFFICIENTS TO ZERO
C*
      CALL RESET(CFT,MNWV2)
C*
C*    LATITUDE LOOP
C*
      DO LAT=1,JMAXHF
C*
C*    SPLIT NORTHERN AND SOUTHERN PORTIONS OF THE GRID DATA
C*
      DO I=1,IMAX
      GN(I)=GRD(I,LAT)
      GS(I)=GRD(I,JMAX-LAT+1)
      ENDDO
C*
C*    COMPUTES FOURIER COEFFICIENTS
C*
      CALL GRDTOS(GN,1,IMX,IMAX,1,IFAX,TRIGS,GWORK)
      CALL GRDTOS(GS,1,IMX,IMAX,1,IFAX,TRIGS,GWORK)
C*
C*    COMPUTES SYMETRIC AND ASYMETRIC PARTS
C*             OF THE FOURIER COEFFICIENTS
C*
      CALL SYMASY(GN,GS,GWORK,IMX,1)
C*
C*    COMPUTES LEGENDRE ASSOCIATED FUNCTIONS (LAF)
C*
      CALL PLN2(PLN,DCOLRA,LAT,DEPS,MEND1,NEND2,JEND2,MNWV1,JMAXHF,LA1)
C*
C*    MULTIPLY LAF BY GAUSSIAN WEIGHTS
C*
      ...
      L=L+1
      LX=LA1(MM,NN)
      QLN(2*L-1)=PLN(LX)*WGT(LAT)
      QLN(2*L  )=PLN(LX)*WGT(LAT)
      ENDDO
      ENDDO
      ELSE
      DO MN=1,MNWV0
      ...
      ENDIF
C*
C*    COMPUTES SPHERICAL HARMONICS COEFFICIENTS
C*
      CALL FL22(GN,GS,CFT,QLN,IMX,MEND1,NEND1,JEND1,MNWV2,1,QWORK)
C*
      ENDDO
C*
      RETURN
