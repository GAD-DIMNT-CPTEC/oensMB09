...
C*
     ...
     *           MNWV2=MEND1*(MEND1+1))
C*
      INTEGER IYRST,IMST,IDST,IYREND,IMEND,IDEND,NDAYS,INDX,
     *        J,I,M,LOND,LATD
      REAL SICE,TICE,UNDEF,EMAX,EMIN,FMAX,FMIN,GMAX,GMIN
      CHARACTER*7 FMTI
     ...
     *     SSTR(ISST,JSST),FLAG(ISST,JSST),CFTOP(MNWV2)
      LOGICAL FLGIN(5),FLGOUT(5)
      DOUBLE PRECISION DWORK(2*LONS)
      INTEGER*4 IFDAY,IDATE(4),IDATEC(4)
      REAL*4 TOD
      REAL*4 SSTGRD(IMAX,JMAX),TOPGRD(IMAX,JMAX),
     *       LSMGRD(IMAX,JMAX),CFTPR(MNWV2)
      NAMELIST /SSTNML/ SICE, TICE
      DATA SICE /-1.749/, TICE /-1.75/
C*
      DATA FMTI/'(   I1)'/
C*
      READ(*,SSTNML)
...
C*
C*    GRID ORIENTATION:
C*    OI SST INPUT DATA: (1,1) = (179.5W,89.5S) 
C*                 (ISST,JSST) = (179.5E,89.5N)
C*
C*    INITIALIZE MSKIN TO 1 TO MAKE INTERPOLATION AT EVERY POINT
C*    AND COMPUTE WEIGHTS
C*
      DO J=1,JSST
      DO I=1,ISST
      MSKIN(I,J)=1
...
C*
      ...
      LSMGRD(I,J)=FLOAT(1-2*LSMASK(I,J))
      ENDDO
      ENDDO
C*
...
C*
      WRITE(52)LSMGRD
C*
C*    LOOP THROUGH 12 MONTHS
C*
      DO M=1,12
C*
C*    WRITE TOPOGRAPHY AND LAND-SEA MASK FOR EVERY
C*    MONTH DUE TO GRADS COMPATIBILITY
C*
      WRITE(54)TOPGRD
      WRITE(54)LSMGRD
C*
...
C*
      READ(50,'(8I5)')IYRST,IMST,IDST,IYREND,IMEND,IDEND,NDAYS,INDX
      READ(50,'(16F5.2)')SSTR
C*
...
C*
      DO J=1,JSST
      DO I=1,ISST
      IF(SSTR(I,J).LT.TICE)SSTR(I,J)=TICE
      FLAG(I,J)=0.0
...
C*
      ...
      ENDDO
C*
      WRITE(6,62)M,EMIN,EMAX,FMIN,FMAX,GMIN,GMAX
   62 FORMAT(' SST Interpolation for Month = ',I4,/,
     *       ' Regular  Grid SST: Min, Max = ',2F8.2,/,
     *       ' Gaussian Grid SST: Min, Max = ',2F8.2/,
     *       ' Masked G Grid SST: Min, Max = ',2F8.2)
C*
C*    WRITE GRADS SST FILE:
C*
      DO J=1,JMAX
      DO I=1,IMAX
      SSTGRD(I,J)=SSTG(I,J)
      ...
      WRITE(54)SSTGRD
C*
C*    WRITE OUT COLA GAUSSIAN GRID CLIMO SST
C*
      WRITE(52)SSTGRD
C*
C*    FINISH MONTH LOOP
