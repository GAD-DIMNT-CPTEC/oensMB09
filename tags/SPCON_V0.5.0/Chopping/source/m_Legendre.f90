MODULE m_Legendre
  ...
  PUBLIC :: Init_Legendre, ClsMem_Legendre, &
            Fourier_To_SpecCoef, Sym_Asy, Get_Conv, Get_Vort, &
            SpecCoef_To_FourierScal, SpecCoef_To_FourierVect

  INTEGER (KIND=Ki4), DIMENSION (:,:), ALLOCATABLE, PUBLIC :: la0, la1

  REAL (KIND=Kr8), DIMENSION (:), ALLOCATABLE, PUBLIC :: glat, colrad, rcs2, snnp1, eps

  REAL (KIND=Kr8), DIMENSION (:,:), ALLOCATABLE, PUBLIC :: qln, qdln, qlnw, qder, qlnwcs, qlnv

  REAL (KIND=Kr8), DIMENSION (:), ALLOCATABLE :: wgt, pln, dpln, der, plnwcs
...
SUBROUTINE Init_Legendre
  ...
  INTEGER (KIND=Ki4) :: j, l, nn, Mmax, mm, lx, mn
  REAL    (KIND=Kr8) :: rd, sn, ea2

  INTEGER (KIND=Ki4), SAVE :: ifp = 1

  IF (ifp == 1) THEN
     CALL SetMem_Legendre
     ifp = 0
  END IF
  ...
  DO j=1,Jmaxhf
     ...
     END DO
  END DO
  WRITE (*, '(/,A)') ' Gaussian Latitudes:'
  WRITE (*, '(6F10.3)') glat(1:Jmaxhf)

END SUBROUTINE Init_Legendre


SUBROUTINE SetMem_Legendre
...
END SUBROUTINE SetMem_Legendre


SUBROUTINE ClsMem_Legendre
...
END SUBROUTINE ClsMem_Legendre
...
20   p1=p2
     ...
     wgt(j)=scale*(1.0_Kr8-COS(rad)*COS(rad))/(p1*p1)
     rcs2(j)=1.0_Kr8/(SIN(rad)*SIN(rad))
  END DO
...
SUBROUTINE pln2 (lat)
  ...
  REAL (KIND=Kr8), DIMENSION (:), ALLOCATABLE, SAVE :: x, y
  ...
  IF (ifp == 1) THEN
     ...
     ifp=0
     DO mm=1,Mend1
        x(mm)=SQRT(2.0_Kr8*mm+1.0_Kr8)
        y(mm)=SQRT(1.0_Kr8+0.5_Kr8/REAL(mm))
     END DO
     rthf=SQRT(0.5_Kr8)
  END IF
  colr=colrad(lat)
...
SUBROUTINE plnder (rcs2l, wgtl)
  ...
  REAL (KIND=Kr8), INTENT (IN) :: rcs2l, wgtl

  INTEGER (KIND=Ki4) :: n, l, nn, Mmax, mm, mn, lm, l0, lp
  REAL    (KIND=Kr8) :: raa, wcsa

  REAL (KIND=Kr8), DIMENSION (Mnwv1) :: x

  INTEGER (KIND=Ki4), SAVE :: ifp = 1
  REAL    (KIND=Kr8), SAVE :: eriv, ersqiv

  REAL (KIND=Kr8), DIMENSION (:), ALLOCATABLE, SAVE :: an
...
SUBROUTINE Fourier_To_SpecCoef (fp, fm, fln, Ldim, lat)
  ...
  INTEGER (KIND=Ki4), INTENT (IN) :: Ldim, lat

  REAL (KIND=Kr8), DIMENSION (Imx,Ldim),   INTENT (IN ) :: fp, fm
  REAL (KIND=Kr8), DIMENSION (Mnwv2,Ldim), INTENT (OUT) :: fln

  INTEGER (KIND=Ki4) :: k, l, nn, Mmax, mm, mn

  REAL (KIND=Kr8), DIMENSION (Mnwv2) :: s
...
END SUBROUTINE Fourier_To_SpecCoef


SUBROUTINE Sym_Asy (a, b, Ldim)
  ...
  REAL (KIND=Kr8), DIMENSION (Imx,Ldim), INTENT (IN OUT) :: a, b

  INTEGER (KIND=Ki4) :: i, k

  REAL (KIND=Kr8), DIMENSION (Imx,Ldim) :: t
...
END SUBROUTINE Sym_Asy


SUBROUTINE Get_Conv (am, ap, bm, bp, fln, lat)
  ...
  REAL (KIND=Kr8), DIMENSION (Imx,Kmax),   INTENT (IN)  :: am, ap, bm, bp
  REAL (KIND=Kr8), DIMENSION (Mnwv2,Kmax), INTENT (OUT) :: fln
...
SUBROUTINE Get_Vort (am, ap, bm, bp, fln, lat)
...
END SUBROUTINE Get_Vort


SUBROUTINE SpecCoef_To_FourierScal (fln, ap, am, Ldim, lat)
  ...
  REAL (KIND=Kr8), DIMENSION (Mnwv2,Ldim), INTENT (IN)  :: fln
  REAL (KIND=Kr8), DIMENSION (Imx,Ldim),   INTENT (OUT) :: ap, am

  INTEGER (KIND=Ki4) :: l, k, mm, mn, nn, Mmax, Mstr, Mmax1, Mend1d
...
END SUBROUTINE SpecCoef_To_FourierScal


SUBROUTINE SpecCoef_To_FourierVect (fln, ap, am, Ldim, lat)
  ...
  REAL (KIND=Kr8), DIMENSION (Mnwv3,Ldim), INTENT (IN)  :: fln
  REAL (KIND=Kr8), DIMENSION (Imx,Ldim),   INTENT (OUT) :: ap, am

  INTEGER (KIND=Ki4) :: l, k, mm, mn, nn, Mmax, Mstr, Mmax1, Mend1d

  REAL (KIND=Kr8), DIMENSION (Mnwv3) :: s
...
END SUBROUTINE SpecCoef_To_FourierVect
