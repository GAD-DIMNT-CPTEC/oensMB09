...
CONTAINS
  ...
  SUBROUTINE GrpComp(&
    ...
    REAL   , INTENT(IN   ) :: ercossin(ibMax)
    REAL   , INTENT(IN   ) :: fcor   (ibMax)
    REAL   , INTENT(IN   ) :: cosiv  (ibMax)
...
!**(JP)** fim da mudanca para comparacao;
  ...
  SUBROUTINE vadvtmp(tmp, p1, p2, h1, h2, dot, psint, &
    ...
    ELSE
       DO k=1,kMax-1
          DO i=1,ibLim
             w1(i,k) = ( p1(k) - 1.0 ) * tmp(i,k+1)
             w2(i,k+1) = ( 1.0 - p2(k+1) ) * tmp(i,k)
             w3(i,k) = ci(k+1) * psint(i,kMax) - psint(i,k)
          ENDDO
       ENDDO
    ENDIF
