...
!CDIR NODEP
  ...
  SUBROUTINE ImplDifu(dt, mnRIFirst, mnRILast)
    ...
    INTEGER :: mn, k
    REAL :: work(2*mnMax)
    REAL, ALLOCATABLE, DIMENSION(:,:) :: holda, holdb, holdc
    !
    ...
    IF (dodia(nDiag_hdidif)) THEN
       holda=qdivp-holda
       CALL upspec (holda, nDiag_hdidif)
       DEALLOCATE (holda)
    END IF
    !
    ...
    IF (dodia(nDiag_hhedif)) THEN
       holda=qtmpp-holda
       CALL upspec (holda, nDiag_hhedif)
       DEALLOCATE (holda)
    END IF
    IF (dodia(nDiag_hvodif)) THEN
       holda=qrotp-holdb
       CALL upspec (holdb, nDiag_hvodif)
       DEALLOCATE (holdb)
    END IF
    IF (dodia(nDiag_hmodif)) THEN
       holda=qqp-holdc
       CALL upspec (holdc, nDiag_hmodif)
       DEALLOCATE (holdc)
    END IF
!$OMP BARRIER
