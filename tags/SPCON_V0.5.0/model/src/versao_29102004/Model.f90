...
PROGRAM model
  ...
  USE FieldsPhysics, ONLY:     &
       InitFieldsPhyscs, InitVariancia, InitBoundCond, InitCheckfile, InitSurfTemp, InitGetsbc, &
       restartphyscs, ssib, rvisbc, rvisdc, rnirbc, rnirdc, dswtop, gl0, zorl, tseam, imask, rvisb, &
       rvisd, rnirb, rnird, gtsea, tc0, tg0, td0, w0, capac0, tcm, tgm, tdm, wm, capacm, ppli, ppci, &
       prct, prcc, prcp1, prcp2, prcp3, prcpt, toplv, botlv, geshem, convc, convt, convb 
  ...
  USE  Options, ONLY:          &
       InitResSet, InitNamModel,InitBlkdat, cdhl, istrt, filta, filtb, delt, nfin0, nfin1, initlz, &
       nfcnv0, dogwd, doprec, nfprc, isimp, nhdhn, &
       grhflg, igwd, ifalb, ifsst, ifslm, ifsnw, sstlag, intsst,  &
       nftgz0, igrfu, iptu, ighdr, monl, ighou, enhdif, impdif, dt, swint, trint, yrl, &
       idate, idatec, idatef, kt, ktm, ktp, jdt, ddelt, percut, dk, tk, nfout0, nfout1, &
       nfout2, nfsibo, nfsibi, nfnmi, nfdbh, nfdrct, nfdiag, nf3d, nstep, maxstp, &
       isteps, ndord, nfsst, nfcnv1, &
       allghf, vcrit, alpha, dodyn, nfdyn, maxtim, maxtfm, cth0, &
       dct, mdxtfm, ctdh0, dctd, nfdhn, mextfm, cteh0, dcte, neprog, nedrct, nefcst, mxrdcc,  &
       icld, inalb, mxiter, nfvar, nferr, nfprt, ifprt, ifilt, &
       trunc, vert, reststep, fNameInput0, fNameInput1, fNameNmi, &
       fNameSSTAOI, &
       fNameSnow, fNameSoilms, fNameAlbedo, fNameCnftBl, fNameCnf2Tb, fNameLookTb, fNameSibVeg, &
       fNameSibAlb, fNameOrgvar, fNameSibmsk, fNameTg3zrl, start, deltOut, slagr, reducedGrid, &
       linearGrid, nlnminit, diabatic, eigeninit, rsettov, GenRestFiles, rmRestFiles, &
       maxtid, PREFY, PREFX, EXTF, EXDF, EXTH, EXDH, EXTW, EXDW, EXTS,path_in,dirfNameOutput,&
       fNameDTable,IDATEW
  ...
  INTEGER, PARAMETER   :: ncf         = 1000
  INTEGER, PARAMETER   :: ncf2        = 500
  ...
  IF( TRIM(start) == "warm" )THEN
     ...
     nfsibi = nfsibo
  ELSE
     WRITE(labelf,'(I4.4, 3I2.2)' )IDATEW(4),IDATEW(3),IDATEW(2),IDATEW(1)
  ...
  CALL InitDiagnostics(mgaus  ,ngaus  ,mspec  ,nspec   , &
       doprec, dodyn  , colrad , &
  ...
  !
  IF (slagr) THEN
     CALL InitSL (imaxperj, ibmax, jbmax, imax, jmax, kmax, colrad) 
  END IF
  !
  ! Initialize modules 
  !
  CALL InitPlanBoundLayer(kmax, sl, del_in, si(1:kmax), nfprt, nferr)
  ...
  fileout12=TRIM(dirFnameOutput)//"/"//TRIM(namef)//labeli//labelc//&
  ...
  IF(iabs(initlz).GE.1.AND.initlz.GT.-3) THEN
     CALL rsdiag
     CALL Diaten(slagr,fNameInput0,fNameInput1,ifday, tod, idate, idatec,kt,ktm)
     ktm=0
     ktp=0
     kt =0
     !
  ...
  !     
  IF(ifday.EQ.0.AND.tod.EQ.zero.AND.nfout2.NE.0) THEN
  ...
  IF (nlnminit) THEN
     ...
     IF(iabs(initlz).EQ.2) THEN
        ...
        IF (nfout2 .NE. 0) THEN
           ...
           IF (GenRestFiles) THEN
              OPEN (nfout2, FILE=TRIM(fileout12), ACTION='write', STATUS='replace')
              CALL gwrite(nfout2,ifday ,tod   ,idate ,idatec,qlnpp ,qtmpp , &
...
!$OMP END SINGLE
     ...
     END IF
     IF (lreststep) THEN
        IF(istep.EQ.2.AND.nfout2.NE.0)nfout1=nfout2
!$OMP SINGLE
        !
        ...
        IF (rmRestFiles) THEN
           ...
           OPEN (nfout1, FILE=TRIM(fileout12), ACTION='write', STATUS='replace')
        ...
        IF((ifprt(3).EQ.2.AND.istep.EQ.isteps).OR.ifprt(3).GE.3) &
