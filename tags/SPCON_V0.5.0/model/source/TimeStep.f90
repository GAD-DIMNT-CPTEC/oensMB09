...
MODULE ModTimeStep
  ...
  USE Options, ONLY: nscalars, MasCon_ps
...
CONTAINS
  ...
  SUBROUTINE TimeStep (fb1, fa, fb, &
    ...
    USE FieldsDynamics, ONLY: &
         ...
         omg2, fgps, fgtlam, fgtphi, fgqlam, fgqphi, fgulam,  &
         fguphi, fgvlam, fgvphi, fgtlamm, fgtphim, fgplamm, &
         fgpphim, fgdivm, fgzs, fgzslam, fgzsphi, fgqmm, fgumm2, fgvmm2,   &
         qqp, qqt, qrotp, qrott, qtmpp, qdiaten, fgtmpmm2, fgqmm2,fgps2,fgtmpmm1
    ...
    DO
       ...
       CALL grpcomp ( &
            ...
            fgvm    (1,1,jb), fgtmpm  (1,1,jb), fgqm    (1,1,jb), omg2    (1,1,jb), &
    ...
    IF (cdhl(jdt)) THEN
       !$OMP SINGLE
       CALL Collect_Grid_Red(ustr, aux3, 0)
       IF (myid.eq.0) THEN
          CALL WriteDiagHead(nfdhn,ifday,tod)
          CALL WriteField(nfdhn, aux3)
       ENDIF
       CALL Collect_Grid_Red(vstr, aux3, 0)
       IF (myid.eq.0) THEN
          CALL WriteField(nfdhn, aux3)
       ENDIF
       !$OMP END SINGLE
    ...
    IF (slagr)  THEN 
       ...
       !  ----------------------------------------------------------
       !
       CALL SemiLagr (2, dt, nlnminit, inew, iold)
    ELSE
       !
       !  Finish tendencies
       !  -----------------
    ...
    !  ---------------------------------
    !
    CALL DirTrans(rcl, dt, nlnminit, slagr, &
    ...
    IF (.NOT. nlnminit) THEN
       ...
       !  -------------------------------------------------
       !
       CALL SemiImpl(dt, slagr, mnRIFirst, mnRILast)
       !
       ...
       IF (MasCon_ps) THEN
          init = 0
          IF (fb1.eq.1.0_r8) init = 1
          CALL GlobConservation(jFirst, jLast, jbFirst, jbLast, inew, init)
       ENDIF
       !
       ...
       IF(TRIM(isimp).ne.'YES') THEN
          ...
          DO
           ...
           CALL HumidPhysics(jb, fgqmm(1,1,jb), fgtmp(1,1,jb),&
                  fgq(1,1,jb), fgps(1,  jb),fgumm2(1,1,jb)  , &
                  fgvmm2(1,1,jb),omg2(1,1,jb),fgtmpmm1(1,1,jb), &
                  fgtmpmm2(1,1,jb), fgqmm2(1,1,jb),fgps2(1,jb),&
       ...
       END IF
       !
       !  adjust control of passive scalars
       !  ---------------------------------
       IF (fa.eq.0.0_r8) THEN
          iaux = iact
          iact = inew
          inew = iaux
        ELSE
