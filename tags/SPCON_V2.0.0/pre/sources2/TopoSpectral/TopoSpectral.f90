...
!!MARCELO 2
   ...
   OPEN (FILE=TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//nLats, &
         UNIT=nftpi, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRec, &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   OPEN (FILE=TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//Trunc, &
         UNIT=nftpo, FORM='UNFORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//Trunc, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   IF (GrADS) THEN
      ...
      OPEN (FILE=TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//'Rec'//nLats, &
            UNIT=nfout, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRec, &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//'Rec'//nLats, &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
      ...
      OPEN (FILE=TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//'Rec'//nLats//'.ctl', &
            UNIT=nfctl, FORM='FORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirMain)//DirPreOut//TRIM(VarNameT)//'Rec'//nLats//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
...
!!MARCELO 3
  IF (instrument) THEN
     iThread = 0
     CALL DumpWatch(wt(iThread), unitDump,'TempoTopoSpectral')
