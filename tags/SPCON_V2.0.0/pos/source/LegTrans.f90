!
!  $Author: pkubota $
!  $Date: 2006/10/30 18:38:49 $
!  $Revision: 1.2 $
!
MODULE LegTrans

  USE Constants, ONLY : r8, nferr
  USE PrblSize, ONLY : mMax=>Mend1, nMax=>Nend1, &
                       nExtMax=>Mend2, mnMax=>Mnwv2, &
                       mnExtMax=>Mnwv3, Mnwv0, Mnwv1, JmaxHf
  USE SpectralRep, ONLY : eps, la1
  USE GaussRep, ONLY : colrad, wgt, rcs2

  IMPLICIT NONE

  PRIVATE

  PUBLIC :: CreateLegTrans, Spec2Four, Four2Spec, DivRot, SplitTrans

  REAL (KIND=r8), ALLOCATABLE, DIMENSION(:,:) :: LegS2F, LegExtS2F, LegDerS2F
  REAL (KIND=r8), ALLOCATABLE, DIMENSION(:,:) :: LegF2S, LegDerNS, LegDerEW

  INTEGER, ALLOCATABLE, DIMENSION(:) :: LenDiag, LenDiagExt, LastPrevDiag, LastPrevDiagExt
...
CONTAINS
  ...
  SUBROUTINE CreateLegTrans ()
    ...
    ALLOCATE (LegS2F   (mnMax, jMaxHf))
    ALLOCATE (LegDerS2F(mnMax, jMaxHf))
    ALLOCATE (LegExtS2F(mnExtMax, jMaxHf))
    ALLOCATE (LegF2S   (mnMax, jMaxHf))
    ALLOCATE (LegDerNS (mnMax, jMaxHf))
    ALLOCATE (LegDerEW (mnMax, jMaxHf))

    CALL LegPols()

    ! diagonal length

    ALLOCATE (LenDiag(nMax))
    ALLOCATE (LenDiagExt(nExtMax))
    DO diag=1,nMax
       LenDiag(diag)=2*(mMax+1-diag)
    END DO
    LenDiagExt(1) = 2*mMax
    DO diag=2,nExtMax
       LenDiagExt(diag)=2*(mMax+2-diag)
    END DO

    ! last element previous diagonal

    ALLOCATE (LastPrevDiag(nMax))
    ALLOCATE (LastPrevDiagExt(nExtMax))
    DO diag=1,nMax
       LastPrevDiag(diag)=(diag-1)*(2*mMax+2-diag)
    END DO
    LastPrevDiagExt(1)=0
    DO diag=2,nExtMax
       LastPrevDiagExt(diag)=(diag-1)*(2*mMax+4-diag)-2
    END DO
  ...
  SUBROUTINE pln2 (sln, colrad, lat, eps, la1)
    ...
    INTEGER, INTENT(IN) :: la1(mMax,nExtMax)
    ...
    DO mm=1,mMax
       sln(mm+mMax)=x(mm)*sinlat*sln(mm)
    END DO
    DO nn=3,nExtMax
       DO mm=1,mMax+2-nn
          lx=la1(mm,nn)
  ...
  SUBROUTINE plnder (pln, dpln, der, plnwcs, rcs2l, wgtl, eps, la1)
    ...
    USE Constants, ONLY : EMRad1, EMRad12
    ...
    END DO
    DO mm=1,mMax
       der(mm)=-x(mm)*eps(mm+mMax)*pln(mm+mMax)
    END DO
    DO mn=1,Mnwv0
       dpln(mn)=der(mn)
       der(mn)=wcsa*der(mn)
  ...
  SUBROUTINE Spec2Four2D (Spec, Four, der)
    ...
    IF (f1 < 2*mMax) THEN
       WRITE (UNIT=nferr, FMT= '(2A,2I10)') h, &
             ' first dimension of four too small: ', f1, 2*mMax
       STOP
    END IF
    ...
    IF (extended) THEN
       CALL SumSpec (nExtMax, mMax, mnExtMax, f1, f2, jMaxHf, f3, &
                     Spec, LegExtS2F, Four, LenDiagExt, LastPrevDiagExt)
    ELSE IF (derivate) THEN
       CALL SumSpec (nMax, mMax, mnMax, f1, f2, jMaxHf, f3, &
  ...
  SUBROUTINE Four2Spec2D (Four, Spec)
    ...
    CALL SumFour (nMax, mMax, mnMax, f1, f2, jMaxHf, f3, &
  ...
  SUBROUTINE SumDivRot (Nmax, Mmax, Mnwv, Imax, Jmax, Jmaxhf, Kmax, &
                        U, V, Div, Rot, LegNS, LegEW, Len, LastPrev)
    ...
    REAL (KIND=r8), INTENT(OUT) :: Rot(Mnwv,Kmax) ! spectral field
       ...
       DO j=1,Jmaxhf
          DO diag=1,Nmax,2
             DO ele=1,Len(diag)
                ...
                Rot(ele+LastPrev(diag),k)=Rot(ele+LastPrev(diag),k)+ &
  ...
  END SUBROUTINE SumDivRot


  SUBROUTINE DivRot (U, V, Div, Rot)
    ...
    REAL (KIND=r8), INTENT(OUT) :: Rot(:,:) ! spectral field

    INTEGER :: f1, f2, f3

    CHARACTER (LEN=17) :: h="**(DivRot)**"
    ...
    CALL SumDivRot (nMax, mMax, mnMax, f1, f2, Jmaxhf, f3, &
                    U, V, Div, Rot, LegDerNS, LegDerEW, LenDiag, LastPrevDiag)
