  ...
  INTEGER :: i,j,nbs,nmb,ierr,nwk,nhrpwk,rest,statfctrd
  ...
  OPEN(10,FILE=TRIM(probagrf),STATUS='OLD',FORM='FORMATTED')

  CALL attribute(10,undef,imax,jmax,nmembers,nfctdy,freqcalc,nweek,ndacc, &
     	   ...
     	   DO nmb=1,nmembers
     		 ...
     		 DO i=1,imax
     		    IF ( (prcwkac(i,j,nmb) .NE. undef) .AND. (prob(i,j) .NE. undef) ) THEN
     		       IF ( prcwkac(i,j,nmb) .GE. 10.0 ) THEN
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
  ...
  WRITE(unit,'(A)')'*'
  WRITE(unit,'(A)')'VARS  1'
  WRITE(unit,'(A,I2,A)')'PROB10  0 99 PROB. OF ',ndacc,' DAYS ACCUMULATED PRECIPITATION > 10.0 mm' 
...
!+++++Local variables
  ...
  DO nf=1,ngrbs
     ...
     END DO
        ...
        IF (fcthours(nf) .EQ. nhrpwk) THEN
          WRITE(lstoutctl(nwk),'(6A)')'wmaprecprob',anldate(1:10),fctdate(nf)(1:10),'.',TRIM(resol),'.ctl'
          WRITE(*,*)'lstoutctl(',nwk,')=',TRIM(lstoutctl(nwk))
          WRITE(lstoutgrb(nwk),'(5A)')'wmaprecprob',anldate(1:10),fctdate(nf)(1:10),'.',TRIM(resol)
  ...
  END DO
          WRITE(lstoutlst,'(6A)')'wmaprecprob',anldate(1:10),fctdate(ngrbs)(1:10),'.',TRIM(resol),'.lst'
