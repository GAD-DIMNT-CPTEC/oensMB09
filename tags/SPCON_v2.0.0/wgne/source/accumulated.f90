  ...
  REAL               :: lonwtr=0.0,lonetr=360.0,latstr=-90,latntr=90    !latstr=-15.4283,latntr=15.4283
  ...
  OPEN(10,FILE=TRIM(accumsetup),STATUS='OLD',FORM='FORMATTED')
  ...
  DO nf=1,ngrbs
   DO nb=1,nbanllag
    WRITE (*,*) "OIII2", nf, nbanllag,nmembers
     ...
     DO k=1,membm
        IF (nf .EQ. 1) THEN
           WRITE(lstingrb(k+membm,nf,nb),'(A,I2.2,6A)')'GPOS',k,'N',anldate(nb)(1:10),fctdate(nf,nb)(1:10),&
             'P.icn.',TRIM(resol),'.grb'
        END IF
        IF (nf .EQ. 2) THEN
           WRITE(lstingrb(k+membm,nf,nb),'(A,I2.2,6A)')'GPOS',k,'N',anldate(nb)(1:10),fctdate(nf,nb)(1:10),&
             'P.inz.',TRIM(resol),'.grb'
        END IF
        IF (nf .GT. 2) THEN
           WRITE(lstingrb(k+membm,nf,nb),'(A,I2.2,6A)')'GPOS',k,'N',anldate(nb)(1:10),fctdate(nf,nb)(1:10),&
             'P.fct.',TRIM(resol),'.grb'
        END IF
     END DO
        IF (nf .EQ. 1) THEN
           WRITE(lstingrb(nmembers,nf,nb),'(7A)')'GPOS',TRIM(prefx),anldate(nb)(1:10),fctdate(nf,nb)(1:10),&
           'P.icn.',TRIM(resol),'.grb'
        END IF
        IF (nf .EQ. 2) THEN
           WRITE(lstingrb(nmembers,nf,nb),'(7A)')'GPOS',TRIM(prefx),anldate(nb)(1:10),fctdate(nf,nb)(1:10),&
