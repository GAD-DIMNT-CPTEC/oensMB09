...
PROGRAM Chopping
  ...
  USE InputParameters, ONLY: InitParameters, i4, r4, r8, nferr, nfinp, nfprt, nftop, &
                             ...
                             MendInp, KmaxInp, KmaxInpp, MendOut, KmaxOut, MendMin, &
...
SUBROUTINE GDAStoGANL
  ...
  ELSE IF ( IDVCInp == 2_i4 ) THEN
    ...
    SigIInp(1:KmaxInp+1)=SiSl(1        :  KmaxInp+1)/100.0_r4
  ...
  if(myid.eq.0)then
    ...
    WRITE (UNIT=nfprt, FMT='(7F13.6)') SigIInp
    WRITE (UNIT=nfprt, FMT='(/,A)')   'SigLInp:'
    WRITE (UNIT=nfprt, FMT='(7F13.6)') SigLInp
  endif
