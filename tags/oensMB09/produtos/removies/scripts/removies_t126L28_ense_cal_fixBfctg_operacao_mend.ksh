...
# Para rodar siga o exemplo: removies_${CASE}_ense_cal_fixBfctg_operacao.ksh 2005071200 360 024 024 30 096
...
if [ ${1} = "surpresa" ] ; then
  DATAE=`date +'%Y%m%d'`${2}
  DATA=`date -d "${DATAE:0:8} ${DATAE:8:2}:00 24 hours ago" +"%Y%m%d%H"`
else
 DATA=${1}
 FCTEND=${2}
 FCTINI=${3}
...
PATHA=${HOME}
export FILEENV=`find ${PATHA} -maxdepth 3 -name EnvironmentalVariablesOENS -print`
...
DIRDAT=${SC2_suite}/pos/dataout/${CASE}
DIRDATENS=${SC2_suite}/ensmed/dataout/${CASE}/${DATA}
DIRSCR=${HOME_suite}/produtos/removies
DIRRESGRB=${SC2_suite}/produtos/removies/dataout/${CASE}/${DATA}/deterministico/avaens${ANO}${MES}${DIA}
DIRBANG=${SC2_suite}/produtos/removies/dataout/${CASE}/${DATA}/ensmed
DIRBANV=${SC2_suite}/produtos/removies/dataout/${CASE}/${DATA}/vies

mkdir -p $DIRBANV $DIRBANG $DIRRESGRB

DIRGRDMAP=/stornext/home/oper/bin/grads-2.0.a9.oga
GRADS=/stornext/home/oper/bin/grads-2.0.a9.oga
...
############################################################################################
 ...
 echo 
  ...
  ln -s ${DIRDAT}/${DATAFIM}/${PREFIXP}${DATAFIM}${DATAFIM}P.icn.${CASE}.grb ${DIRRESGRB}/ana${DATAFIM}.grb
  echo "say \"= ***ANALISE =-> ana${DATAFIM}.grb\"" 
  else
  ln -s ${DIRDAT}/${DATAFIM}/${PREFIXA}${DATAFIM}${DATAFIM}P.icn.${CASE}.grb ${DIRRESGRB}/ana${DATAFIM}.grb
  echo "say \"= ***ANALISE =-> ana${DATAFIM}.grb\""	
  fi
   ...
   while ((${fct} <= ${FCTFIX})) ; do
    ...
    DATAINIC=`date -d "${DATAFIM:0:8} ${DATAFIM:8:2}:00 ${fct} hours ago" +"%Y%m%d%H"`
    ...
    if ((${DATAINIC} >= 2005091400)); then
     if ((${DATAINIC} != ${DATAFIM})); then
      ln -s ${DIRDAT}/${DATAINIC}/${PREFIXP}${DATAINIC}${DATAFIM}P.fct.${CASE}.grb ${DIRRESGRB}/prev${fct}${DATAFIM}.grb
      echo "say \"= PREV =-> prev${fct}${DATAFIM}.grb\""
     else
      ln -s ${DIRDAT}/${DATAINIC}/${PREFIXP}${DATAINIC}${DATAFIM}P.icn.${CASE}.grb ${DIRRESGRB}/prev${fct}${DATAFIM}.grb
      echo "say \"= PREV =-> prev${fct}${DATAFIM}.grb\""
     fi
    else
     if ((${DATAINIC} != ${DATAFIM})); then
      ln -s ${DIRDAT}/${DATAINIC}/${PREFIXA}${DATAINIC}${DATAFIM}P.fct.${CASE}.grb ${DIRRESGRB}/prev${fct}${DATAFIM}.grb
      echo "say \"= PREV =-> prev${fct}${DATAFIM}.grb\"" 
     else
      ln -s ${DIRDAT}/${DATAINIC}/${PREFIXA}${DATAINIC}${DATAFIM}P.icn.${CASE}.grb ${DIRRESGRB}/prev${fct}${DATAFIM}.grb
      echo "say \"= PREV =-> prev${fct}${DATAFIM}.grb\"" 
     fi
    fi
    let fct=$fct+$FCTINT
   done
DATACTL=`date -d "${DATA:0:8} ${DATA:8:2}:00 0 days" +"%HZ%d%b%Y"`
...
undef 9.999E+20
*
title PRESSURE HISTORY    CPTEC AGCM REVIS 1.0 2000  ${CASE}  COLD
*
dtype grib   255
*
options yrev
...
EOF2
 ...
 ${GRADS}/grads -${batch}lc "run ${DIRSCR}/BiasFile${fct}h.gs"
...
mkdir -p ${DIRBANV}
mv -f ${DIRRESGRB}/BiasFile*h${DATA}*  ${DIRBANV}
...
############################################################################################
  ...
  while ((${numdias} <= ${DIASDEVIES})) ; do
   ...
   while ((${fct} <= ${FCTFIX})) ; do
    ...
    ln -s ${DIRBANV}/BiasFile${fct}h${DATAFIM} ${DIRRESGRB}/bias${fct}h${DATAFIM}
    echo "say \"= BIAS =-> bias${fct}h${DATAFIM}\"" 
    let fct=$fct+$FCTINT
   done
   DATAFIM=`date -d "${DATAFIM:0:8} ${DATAFIM:8:2}:00 1 day ago" +"%Y%m%d%H"`
   let numdias=numdias+1
  done
        
DATACTL=`date -d "${DATAFIM:0:8} ${DATAFIM:8:2}:00 1 day" +"%HZ%d%b%Y"`
...
EOF2
 ...
 ${GRADS}/grads -${batch}lc "run ${DIRSCR}/BiasMean${fct}h.gs"
...
# BATCH para rodar o grads em background
 batch="b"
 DATAFIMPREV=`date -d "${DATAINIPREV:0:8} ${DATAINIPREV:8:2}:00 ${fct} hours" +"%Y%m%d%H"`
...
echo "'open ${DIRDATENS}/${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.fct.${CASE}.ctl' " >> ${DIRSCR}/RemoFile${fct}h.gs
echo "'set gxout fwrite' " >> ${DIRSCR}/RemoFile${fct}h.gs
echo "'set fwrite ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.fct.${CASE}' " >> ${DIRSCR}/RemoFile${fct}h.gs
else
echo "'open ${DIRDATENS}/${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.icn.${CASE}.ctl' " >> ${DIRSCR}/RemoFile${fct}h.gs
echo "'set gxout fwrite' " >> ${DIRSCR}/RemoFile${fct}h.gs
echo "'set fwrite ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.icn.${CASE}' " >> ${DIRSCR}/RemoFile${fct}h.gs
fi

cat << EOF2 >> ${DIRSCR}/RemoFile${fct}h.gs 
*AMM  'set gxout fwrite'
*AMM  'set fwrite ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.fct.${CASE}'
...
EOF2
 ...
 echo "LALALALALALA"
 echo "${GRADS}/grads -${batch}lc run ${DIRSCR}/RemoFile${fct}h.gs"
 ${GRADS}/grads -${batch}lc "run ${DIRSCR}/RemoFile${fct}h.gs"
...
#####################################################################
 ...
 while ((${fct} <= ${FCTEND})) ; do 
 DATAFIMPREV=`date -d "${DATAINIPREV:0:8} ${DATAINIPREV:8:2}:00 ${fct} hours" +"%Y%m%d%H"`
 DATACTLPREV=`date -d "${DATAFIMPREV:0:8} ${DATAFIMPREV:8:2}:00 0 days" +"%HZ%d%b%Y"`

if ((${DATAINIPREV} != ${DATAFIMPREV})); then

cat << EOF1 >  ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.fct.${CASE}.ctl
dset  ^${PREFIXSAI}${DATAINIPREV}${DATAFIMPREV}p.fct.${CASE}
undef 9.999E+20
xdef   384 linear    0.000   0.9375000000
ydef   192 levels 
 -89.28423 -88.35700 -87.42430 -86.49037 -85.55596 -84.62133 -83.68657 -82.75173
...
ls -lrt ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.fct.${CASE}.ctl | awk '{print $9" "$10" "$11}'

else

cat << EOF1 >  ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.icn.${CASE}.ctl
dset  ^${PREFIXSAI}${DATAINIPREV}${DATAFIMPREV}p.icn.${CASE}
undef 9.999E+20
xdef   384 linear    0.000   0.9375000000
ydef   192 levels 
 -89.28423 -88.35700 -87.42430 -86.49037 -85.55596 -84.62133 -83.68657 -82.75173
...
ls -lrt ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.icn.${CASE}.ctl | awk '{print $9" "$10" "$11}'
...
let fct=$FCTINI
 while ((${fct} <= ${FCTEND})) ; do
  ...
  DATAFIMPREV=`date -d "${DATAINIPREV:0:8} ${DATAINIPREV:8:2} ${fct} hours" +"%Y%m%d%H"`

  if ((${DATAINIPREV} != ${DATAFIMPREV})); then
    mv ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.fct.${CASE}.ctl  ${DIRRESGRB}/${PREFIXSAI}${DATAINIPREV}${DATAFIMPREV}p.fct.${CASE}.ctl  
    mv ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.fct.${CASE}      ${DIRRESGRB}/${PREFIXSAI}${DATAINIPREV}${DATAFIMPREV}p.fct.${CASE}  
  else
    mv ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.icn.${CASE}.ctl  ${DIRRESGRB}/${PREFIXSAI}${DATAINIPREV}${DATAFIMPREV}p.icn.${CASE}.ctl  
    mv ${DIRRESGRB}/RemoFile${PREFIX}${DATAINIPREV}${DATAFIMPREV}P.icn.${CASE}      ${DIRRESGRB}/${PREFIXSAI}${DATAINIPREV}${DATAFIMPREV}p.icn.${CASE}  
  fi
...
mkdir -p ${DIRBANG}
mv -f ${DIRRESGRB}/${PREFIXSAI}${DATAINIPREV}* ${DIRBANG}
