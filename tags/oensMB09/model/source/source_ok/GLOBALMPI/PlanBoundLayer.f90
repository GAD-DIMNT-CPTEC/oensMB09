...
MODULE PlanBoundLayer
  ...
  REAL(KIND=r8),    PARAMETER :: a1    =   0.92_r8 ! 0.78
  REAL(KIND=r8),    PARAMETER :: a2    =   0.74_r8 ! 0.79
  REAL(KIND=r8),    PARAMETER :: b1    =  16.6_r8  ! 15.0
  REAL(KIND=r8),    PARAMETER :: b2    =  10.1_r8  ! 8.0
  REAL(KIND=r8),    PARAMETER :: c1    =   0.08_r8 ! 0.056
...
CONTAINS
  ...
  SUBROUTINE InitPlanBoundLayer(kmax, sl, delsig, si)
    INTEGER, INTENT(IN) :: kmax
    REAL(KIND=r8),    INTENT(IN) :: sl(kmax)
    REAL(KIND=r8),    INTENT(IN) :: delsig(kmax)
    REAL(KIND=r8),    INTENT(IN) :: si(kmax)
    INTEGER  :: k
    REAL(KIND=r8)     :: gam1
    REAL(KIND=r8)     :: gam2
    REAL(KIND=r8)     :: gbyr
    ...
    DO k = 1, kmax
       ...
       con0  (k)=gasr*delsig(k)/(grav*sl(k))
    END DO
    a0    (kmax)=0.0_r8
    b0    (   1)=0.0_r8
    sigr  (kmax)=0.0_r8
    ...
    DO k = 1, kmax-1
       ...
       a0(k)=gbyr*si(k+1)**2/(delsig(k  )*(sl(k)-sl(k+1)))
       b0(k+1)=gbyr*si(k+1)**2/(delsig(k+1)*(sl(k)-sl(k+1)))
    END DO
  ...
  SUBROUTINE ympbl0 ( &
    ...
    REAL(KIND=r8) :: Pbl_PotTintf(ncols,kmax) ! in the inteface between layers
...
!    REAL(KIND=r8) :: Pbl_CoefKm(ncols,kmax)
    ...
    IF(PBL == 1)THEN
       ...
       DO k = 1, kmax-1
          DO i = 1, ncols
             Pbl_PotTintf(i,k)=t0(k)*Pbl_PotTep(i,k)+t1(k)*Pbl_PotTep(i,k+1)
          END DO
       END DO
       ...
       ! grav / inverse of distance between layers
       ! 
       ! k-th value measured from middle of layer k to middle of layer k+1
       ! Hence it is centered on the interface between layers
       !
       !   dP = - rho g dz
       !   P = rho R T
       !
       ...
       !   1/dz = -g/(T*R) * P/dP
       !
       ! But init() routine calculated:
       !
       !                   grav       si(k+1)      
       !   con2(k)=grav * ------ * ---------------- = grav * sqrt(con1)
       !                   gasr     sl(k)-sl(k+1)) 
       !
       ! Therefore:
       !
       !   g/dz(k)    = con2(k) / ATemp(k)
       !   1/dz(k)**2 = con1(k) / ATemp(k)**2
       !
       DO k = 1, kmax-1
          DO i = 1, ncols
             Pbl_gDz(i,k)=con2(k)/Pbl_ATemp(i,k)
             Pbl_IDz2(i,k)=con1(k)*Pbl_Itemp(i,k)
          END DO
       END DO

       ! TKE Production rate due to buoyancy
       !
       !               g    dThv 
       !   Pb = - Kh ----- ------
       !              Thv    dz
       !
       ! The related coefficient is:
       !
       !           l^2    g    dThv 
       !   Gh = - ----- ----- ------
       !           q^2   Thv    dz
       !
       ! where q^2 = 2*TKE and l is the eddy mixing length (m)
       !   Here we calculate only the common terms of both equations.
       !   Notice that dThv is calculated using values in the middle
       ! of adjacent layers, while Thv has to be interpolated
       ! to the interface in-between.

       DO k = 1, kmax-1
          DO i = 1, ncols
             Pbl_Stabil(i,k)=Pbl_gDz(i,k)*(Pbl_PotTep(i,k+1)-Pbl_PotTep(i,k)) &
                  /Pbl_PotTintf(i,k)
          END DO
       END DO

       ! TKE Production rate due to shear
       !
       !                      2           2
       !           {  |  dU  |    |  dV  |  }
       !   Ps = Km {  | -----|  + | -----|  }
       ...
       DO k = 1, kmax-1
          DO i = 1, ncols
             Pbl_Shear(i,k)=csqiv(i)*Pbl_IDz2(i,k)*&
                  ((gu(i,k)-gu(i,k+1))**2+(gv(i,k)-gv(i,k+1))**2)
             Pbl_Shear(i,k)=MAX(Pbl_Shear(i,k),1.0e-5_r8)
          END DO
       END DO

       ! RICHARDSON NUMBER
       !
       ! Gradiente/bulk (large scale) Richardson number is given by
       !
       !   Ri = - Gh/Gm = Rf (Sm/Sh) == pbl_stabil / pbl_shear
       !
       ! Flux Richardson number is given by
       !
       !   Rf = - Pb/Ps
       !
  ...
  SUBROUTINE VDIFV(kMax,nCols,twodti,Pbl_ITemp,Pbl_CoefKm,a,b,gu,gv,gmu)
    ...
    REAL(KIND=r8),    INTENT(in   ) :: Pbl_CoefKm(nCols,kMax)!(m/sec**2)
    REAL(KIND=r8),    INTENT(in   ) :: gu        (nCols,kMax)
    REAL(KIND=r8),    INTENT(in   ) :: gv        (nCols,kMax)
    REAL(KIND=r8),    INTENT(inout) :: gmu       (nCols,kMax,4)

    REAL(KIND=r8) :: Pbl_CoefKm2(nCols,kMax)
    ...
    DO k = 1, kMax-1
       DO i = 1, nCols
          Pbl_CoefKm2(i,k)=Pbl_CoefKm(i,k)*Pbl_ITemp(i,k)!(m/sec**2) * (1/K**2) = m/(sec**2 * K**2)
          !
          !                   --   --      -- -- 2
          !                  |       |    |  1  |         K**2 * s
          !a(k)=             |  2*Dt |  * | --- |     ==>-----------
          ...
          !                   --   --      -- --
          !                                         K**2 * s         m              1
          Pbl_KHbyDZ(i,k  )=a(k  )*Pbl_CoefKm2(i,k)!----------- * -------------  = --------
          !                                           m**2          s**2*K**2     m * s
          !
          !                                         K**2 * s        m**2             1
          Pbl_KMbyDZ(i,k+1)=b(k+1)*Pbl_CoefKm2(i,k)!----------- * -------------  = --------
          !                                           m**2          s**2*K**2         s
          !
