  PROGRAM tmed_clim_inmet
  
  IMPLICIT NONE
  
!
! 
! This program makes the interpolation of the climatological tsmtipitation
...
!itmed: number of the tsmtipitation record in the climatological files
!itmed: number of the TIME MEAN ABSOLUTE TEMPERATURE in the climatological files
 

  INTEGER, PARAMETER :: loni=176,lati=160,nsph=86400 
  INTEGER, PARAMETER :: itmed=1                      
  ...
  DATA fnmi  /'cl01te.bin','cl02te.bin', &
              'cl03te.bin','cl04te.bin', &
	      'cl05te.bin','cl06te.bin', &
              'cl07te.bin','cl08te.bin', &
	      'cl09te.bin','cl10te.bin', &
	      'cl11te.bin','cl12te.bin'/
...
!
  ...
  dirclm='/gfs/dk20/modoper/tempo/global/oens/clima/INMET/'

!
! Name of the day climatology. Mean over the period   
!

  WRITE(fnmd,'(A,2I10.10)')'clim_temp_inmet',date1,date3
...
!
  ...
  DO WHILE ( date .LE. date3 )
    ...
    OPEN(10, FILE=TRIM(dirclm)//TRIM(fnmi(mf)), STATUS='OLD', FORM='UNFORMATTED', READONLY,  & 
             ACCESS='DIRECT', RECL=loni*lati, CONVERT='LITTLE_ENDIAN')
    OPEN(11, FILE=TRIM(dirclm)//TRIM(fnmi(mn)), STATUS='OLD', FORM='UNFORMATTED', READONLY,  & 
             ACCESS='DIRECT', RECL=loni*lati, CONVERT='LITTLE_ENDIAN')

    READ(10,REC=itmed)((bufa(i,j),i=1,loni),j=1,lati)
    READ(11,REC=itmed)((bufb(i,j),i=1,loni),j=1,lati)
...
!
  ...
  WRITE(20,'(A)')'*'
  WRITE(20,'(A)')'VARS 1'
  WRITE(20,'(A)')'TMED  0 99 TEMPERATURA MEDIA (C)'
  WRITE(20,'(A)')'ENDVARS'
