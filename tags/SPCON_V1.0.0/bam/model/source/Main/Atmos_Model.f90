...
CONTAINS
  ...
  USE  Options, ONLY: &
       ...
       jdt,ddelt,ifsst,ifco2flx,intsst,sstlag,ktm,kt,yrl,ktp,&
       ifslm,intsoilm,soilmlag,&
       ...
       nfcnv0,nfdiag,nffcst,nftmp,nedrct,neprog,nefcst,nfcnv1,&
       ...
       fNameDTable, fNameSibAlb,fNameSibmsk ,fNameMicro,fNameSoilms,&
  ...
  USE FieldsDynamics, ONLY: &
       ...
       fgpass_scalars, adr_scalars, fgice, fgicem, fgliq, fgliqm, fgvar, fgvarm
...
! add solange 27-01-2012
       ...
       sm0,mlsi,   &
       AlbNirBeam, AlbNirDiff, AlbVisBeam, AlbVisDiff,sfc
...
!  END IF
  ...
  ELSE
     ...
     IF (microphys) THEN
        IF((nClass+nAeros)>0)THEN
           READ(UNIT=nfin0) fgicem, fgice, fgliqm, fgliq, fgvarm,fgvar
        ELSE
           READ(UNIT=nfin0) fgicem, fgice, fgliqm, fgliq
        END IF
     END IF  
     IF (slagr.or.slhum) THEN
     ...
     READ(UNIT=nfin1) qgzs,qlnpp,qtmpp,qdivp,qrotp,qqp,total_mass,total_flux
     qgzs_orig = qgzs
     REWIND nfin0
     REWIND nfin1
  END IF
  ...
  IF(ifday.EQ.0.AND.tod.EQ.zero) THEN
     ...
     CALL wrprog ( &
          ...
          qrotp    ,qdivp   ,qqp        ,qlnpp     ,qtmpp  ,z0     , &
...
! fim add
  ...
  USE  Options, ONLY: &
       ...
       nfdrct,nfout0,nfin1,nfprt,nfsibo,nfout1,& 
       nefcst,nfdiag, nfcnv1,nedrct,neprog,nffcst,nftmp, &
       nfghou,nfdyn,nfprc ,&
       EXDW ,EXTW ,&
       cthl,&
...
!     END IF
     ...
     IF (lreststep) THEN
        ...
        WRITE(UNIT=nfout1) qgzs_orig,qlnpp,qtmpp,qdivp,qrotp,qqp,total_mass,total_flux
        WRITE(UNIT=nfout0) fgqm, fgum, fgvm, fgtmpm, fgdivm, fglnpm, fgtlamm, &
             fgtphim, fgplamm, fgpphim
        IF (slhum) WRITE(UNIT=nfout0) fgq
        IF (microphys) THEN
           IF((nClass+nAeros) >0 )THEN
              WRITE(UNIT=nfout0) fgicem, fgice, fgliqm, fgliq, fgvarm,fgvar
           ELSE
              WRITE(UNIT=nfout0) fgicem, fgice, fgliqm, fgliq
           END IF
        END IF 
        IF (slagr.or.slhum) THEN
