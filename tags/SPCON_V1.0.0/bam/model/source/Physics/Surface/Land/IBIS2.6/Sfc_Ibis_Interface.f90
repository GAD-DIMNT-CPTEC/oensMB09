MODULE Sfc_Ibis_Interface
  ...
  USE FieldsPhysics, ONLY: Tsfc0,Qsfc0,Tsfcm,Qsfcm,w0,wm,capac0,capacm,td0,tdm,tcm,tc0,tgm,tg0,qm0,tm0,sheleg,gco2flx
...
!      sfc_sice1
  ...
  USE Options, ONLY: OCFLUX,omlmodel,oml_hml0,SLABOCEAN,co2val,atmpbl,ICEMODEL,nClass,nAeros,indexchem,typechem
...
CONTAINS
  ...
  SUBROUTINE Ibis_Interface(intg                ,istrt             , &
    ...
    !INTEGER :: iyearout 
    INTEGER :: nLndPts
    INTEGER :: i,k,ind,nint,IntSib,itr,itrac
   ...
   CALL seasfc( &
          ...
          DO i=1,nCols
             ...
             ELSE
                IF(iMask(i) >= 1_i8)THEN 
                   ...
                   ! local ! instantaneous net ecosystem exchange of co2 per timestep (kg_C m-2/timestep)
                   ! (mol-CO2 / m-2 / second) to (kg-CO2 / m-2 / second)
                   cflx  (i,indexchem(itrac)) = -(tneetot (nLndPts,jb)*(0.044_r8))
                   !cflx  (i,itrac) = (tgpptot (nLndPts)/0.044_r8)
                ELSE
                   cflx  (i,indexchem(itrac)) = gco2flx(i,jb)!*(0.044_r8)
               END IF
             END IF
          END DO
       END IF
    ...
    IF(dodia(nDiag_insnee))THEN ! instantaneous net ecosystem exchange of co2 per timestep (kg_C m-2/timestep)
       ...
       DO i=1, nCols
          IF(iMask(i) >= 1_i8)THEN  
             nLndPts=nLndPts+1
             diag(i) = -tneetot (nLndPts,jb)*(0.044_r8)
          ELSE
             diag(i) = gco2flx(i,jb)!*(0.044_r8)
          END IF
       END DO
