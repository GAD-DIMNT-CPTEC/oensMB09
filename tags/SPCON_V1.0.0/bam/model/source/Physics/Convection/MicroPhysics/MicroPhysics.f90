         ...
         IF (microphys) THEN
            ...
            IF((nClass+nAeros)>0 .and. PRESENT(gvarm))THEN
               ...
               IF(TRIM(ILCON).EQ.'MORR'.or.TRIM(ILCON).EQ.'HUMO'.OR.  TRIM(ILCON).EQ.'HWRF')THEN
                  ...
                  IF( TRIM(ILCON).EQ.'HWRF')THEN
                      nifa3 (i,k,latco) = MAX(nifa3 (i,k,latco),0.0e-12_r8)
                  ELSE
                      NS3   (i,k,latco) = MAX(NS3 (i,k,latco),0.0e-12_r8)
                  END IF
                  NR3 (i,k,latco) = MAX(NR3 (i,k,latco),0.0e-12_r8)
                  IF( TRIM(ILCON).EQ.'HWRF')THEN
                     nwfa3 (i,k,latco) = MAX(nwfa3 (i,k,latco),0.0e-12_r8)
                  ELSE
               ...
               IF(TRIM(ILCON).EQ.'MORR'.or.TRIM(ILCON).EQ.'HUMO'.OR.  TRIM(ILCON).EQ.'HWRF')THEN
                  ...
                  IF( TRIM(ILCON).EQ.'HWRF')THEN
                     nifa2 (i,k,latco) = MAX(nifa2 (i,k,latco),0.0e-12_r8)
                  ELSE
                     NS2 (i,k,latco) = MAX(NS2 (i,k,latco),0.0e-12_r8)
                  END IF
                  NR2 (i,k,latco) = MAX(NR2 (i,k,latco),0.0e-12_r8)
                  IF( TRIM(ILCON).EQ.'HWRF')THEN
                     nwfa2 (i,k,latco) = MAX(nwfa2 (i,k,latco),0.0e-12_r8)
                  ELSE
                     NG2 (i,k,latco) = MAX(NG2 (i,k,latco),0.0e-12_r8)
                  END IF
                  IF(TRIM(ILCON).EQ.'HUMO'.or.TRIM(ILCON).EQ.'HWRF')NC2 (i,k,latco) = MAX(NC2 (i,k,latco),0.0e-12_r8)
               END IF
            END IF
         END IF
         tLrgs(i,k)=t3(i,k)
