...
!    CLOSE(nNameList)
  ...
  IMPLICIT NONE
    ...
    CHARACTER(LEN=10)                 :: LABELC
    CHARACTER(LEN=10)                 :: LABELF
    WRITE(*,*)'pk1',IDATEI,IDATEW,IDATEF
    WRITE(LABELI,'(i4.4,3i2.2)')IDATEI(4),IDATEI(3),IDATEI(2),IDATEI(1)
    WRITE(LABELC,'(i4.4,3i2.2)')IDATEW(4),IDATEW(3),IDATEW(2),IDATEW(1)
    WRITE(LABELF,'(i4.4,3i2.2)')IDATEF(4),IDATEF(3),IDATEF(2),IDATEF(1) 
    WRITE(*,*) LABELI,LABELC,LABELF
    IF ( TRIM(START) == 'cold' ) THEN
       EXTS='S.unf'
       EXTW='F.unf'
       EXDW='F.dir'
  ...
  SUBROUTINE FileNameRest(PRC,OPT)
   IMPLICIT NONE
    ...
    CHARACTER(LEN=10)                 :: LABELC
    CHARACTER(LEN=10)                 :: LABELF
    WRITE(*,*)'pk2',idate,idatec
    WRITE(LABELI(1:10),'(i4.4,3i2.2)')idate(4),idate(2),idate(3),idate(1)
    WRITE(LABELC(1:10),'(i4.4,3i2.2)')idatec(4),idatec(2),idatec(3),idatec(1)
    WRITE(LABELF(1:10),'(i4.4,3i2.2)')idatec(4),idatec(2),idatec(3),idatec(1)
    IF (OPT==1) THEN
  ...
  SUBROUTINE FileNameGeral(PRC,OPT)
    ...
    CHARACTER(LEN=10)                :: LABELII
    CHARACTER(LEN=10)                :: LABELCC
    CHARACTER(LEN=10)                :: LABELFF
    WRITE(*,*)'pk3',IDATEI,IDATEW,IDATEF,PRC,OPT,EXTW,EXDH
    WRITE(LABELII,'(i4.4,3i2.2)')IDATEI(4),IDATEI(3),IDATEI(2),IDATEI(1)
    WRITE(LABELCC,'(i4.4,3i2.2)')IDATEW(4),IDATEW(3),IDATEW(2),IDATEW(1)
    WRITE(LABELFF,'(i4.4,3i2.2)')IDATEF(4),IDATEF(3),IDATEF(2),IDATEF(1)
    WRITE(*,*) LABELII,LABELCC,LABELFF
    IF(TRIM(OPT)=='GDHN')THEN
       FNameGDHN=TRIM(dirFnameOutput)//"/"//TRIM(OPT)//TRIM(PREFX)//labelii//labelcc//EXTW//'.'//TRIM(TruncLev)
    ELSE IF (TRIM(OPT)=='GDYN') THEN
       FNameGDYN=TRIM(dirFnameOutput)//"/"//TRIM(OPT)//TRIM(PREFX)//labelii//labelff//&
                 &EXTW//'.'//TRIM(TruncLev)
    ELSE IF (TRIM(OPT)=='GPRC') THEN
       FNameGPRC=TRIM(dirFnameOutput)//"/"//TRIM(OPT)//TRIM(PREFX)//labelii//labelcc//&
          &EXTW//'.'//TRIM(TruncLev)    
    ELSE IF (TRIM(OPT)=='GFGH') THEN
       FNameOutGH=TRIM(dirFnameOutput)//"/"//TRIM(OPT)//TRIM(PREFX)//labelii//labelcc//EXTW//'.'//TRIM(TruncLev)
       FNameTopGH=TRIM(dirFnameOutput)//"/"//TRIM(OPT)//TRIM(PREFX)//labelii//labelcc//'F.top'//'.'//TRIM(TruncLev)
       FNamenDrGH=TRIM(dirFnameOutput)//"/"//TRIM(OPT)//TRIM(PREFX)//labelii//labelcc//EXDH//'.'//TRIM(TruncLev)
    END IF 
