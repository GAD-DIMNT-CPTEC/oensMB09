...
CONTAINS
  ...
  SUBROUTINE cumulus_driver (&
   ...
   !---------------------------------------------------------
    ...
    IF(TRIM(iccon).EQ.'ARA'.OR.TRIM(iccon).EQ.'KUO'.OR.TRIM(iccon).EQ.'GRE')THEN
       DO i=1,iMax
          ppci  (i)=2.0e0_r8*1.0e3_r8*RAINCV(i)
       END DO    
       !
       ...
       DO k=1,kMax
          DO i=1,iMax
             clheat(i,k)=fac*rdt*(t33(i,k)-Work_Temp(i,k))
          END DO
       END DO

       DO k=1,kMax
          DO i=1,iMax
             cmchan(i,k)=fac*rdt*(cu_umes(i,k)-Work_Umes(i,k))
          END DO
       END DO
       !
       !-----------
    ...
    IF(TRIM(ISCON).EQ.'TIED' .or. TRIM(ISCON).EQ.'SOUZ')THEN
       ...
       DO k=1,kMax
          DO i=1,iMax
             sclhea(i,k)=fac*rdt*(tc(i,k)-t33(i,k))
             scmcha(i,k)=fac*rdt*(qc(i,k)-q33(i,k))
          END DO
       END DO
    END IF
    ...
    IF(TRIM(ILCON).EQ.'YES') THEN
       DO i=1,iMax          
          IF(0.35_r8*cu_temp(i,1)+0.65_r8*cu_temp(i,2).LE.273.2_r8)THEN
             snowflg(i)=Cuml_Rain(i)
             snowfl (i)=fac2*rdt*1.0e3_r8*Cuml_Rain(i)
          END IF
       ...
       DO k=1,kMax
          DO i=1,iMax
             lslhea(i,k)=fac*rdt*(cu_temp(i,k)-tc(i,k))
             lsmcha(i,k)=fac*rdt*(cu_umes(i,k)-qc(i,k))
          END DO
       END DO
    ...
    IF (StartStorDiag)THEN
       CALL ConvecDiagnStorage (&
                      ...
                      fdqn     ,RAINCV   ,Cuml_Rain,snowfl   ,sclhea   , &
    ...
    IF(ghl_local)THEN 
       CALL ConvecGridHistStorage(&
                      iMax     ,kMax     ,latco    ,rdt      ,fac2     , &
                      RAINCV   ,Cuml_Rain,snowflg  ,sclhea   ,scmcha   , &
  ...
  SUBROUTINE ConvecGridHistStorage(&
                   ...
                   rrr      ,Cuml_Rain,snowflg  ,sclhea   ,scmcha   , &
                   clheat   ,cmchan   ,lslhea   ,lsmcha) 
    ...
    IF(TRIM(iccon).EQ.'ARA'.OR.TRIM(iccon).EQ.'KUO'.OR.TRIM(iccon).EQ.'GRE')THEN
          ...
          IF(dogrh(nGHis_clheat,latco))CALL StoreGridHistory(clheat(1:nCols,:),nGHis_clheat,latco)
          IF(dogrh(nGHis_cvmosr,latco))CALL StoreGridHistory(cmchan(1:nCols,:),nGHis_cvmosr,latco)
    END IF

    IF(TRIM(ISCON).EQ.'TIED' .or. TRIM(ISCON).EQ.'SOUZ')THEN
          IF(dogrh(nGHis_sclhea,latco))CALL StoreGridHistory(sclhea,nGHis_sclhea,latco)
          IF(dogrh(nGHis_shcvmo,latco))CALL StoreGridHistory(scmcha,nGHis_shcvmo,latco)
    END IF
    !---------------
    ...
    IF(TRIM(ILCON).EQ.'YES') THEN
          ...
          IF(dogrh(nGHis_snowfl,latco))CALL StoreGridHistory(snowflg  ,nGHis_snowfl,latco,fac2*rdt*1.0e3_r8)
          IF(dogrh(nGHis_sslaht,latco))CALL StoreGridHistory(lslhea   ,nGHis_sslaht,latco)
          IF(dogrh(nGHis_spstms,latco))CALL StoreGridHistory(lsmcha   ,nGHis_spstms,latco)
    END IF
  END SUBROUTINE ConvecGridHistStorage

  SUBROUTINE ConvecDiagnStorage(&
                   ncols    ,kMax     ,latco    ,rdt      ,fac2     , &
                   fdqn     ,rrr      ,Cuml_Rain,snowfl   ,sclhea   , &
                   scmcha   ,clheat   ,cmchan   ,lslhea   ,lsmcha     )    
