  ...
  SUBROUTINE mstad2(ps, sig, tin, tmst, qmst, ktop,  &
    ...
    DO i=1,ncols
       IF(llift(i)) THEN
          ...
          IF(jt(i).LT.1) CALL FatalError(h//&
               "Theta_e table: temp < low limit")
          IF(jt(i).GE.the_ntemp) CALL FatalError(h//&
               "Theta_e table: temp > high limit")
       ENDIF
    END DO
    ...
    DO i=1,ncols
       IF(llift(i)) THEN
          ...
          IF(kp(i).LT.1) CALL FatalError(h//&
               "Theta_e table: pres < low limit")
          IF(kp(i).GE.the_npres) CALL FatalError(h//&
               "Theta_e table: pres > high limit")
       ENDIF
    END DO
