MODULE m_Fourier
  ...
  USE m_Parameters, ONLY : Ki4, Kr8, Imax => ImaxOut, &
                         Ifmx => MFactorFourier,  &
                         Imxt => MTrigsFourier
  ...
  PUBLIC :: Init_FFT, ClsMem_FFT, fft991

  INTEGER (KIND=Ki4), DIMENSION (:), ALLOCATABLE :: ifax

  REAL (KIND=Kr8), DIMENSION (:), ALLOCATABLE :: trigs
...
END SUBROUTINE SetMem_FFT


SUBROUTINE ClsMem_FFT
...
SUBROUTINE fax (n, mode)
  ...
  INTEGER (KIND=Ki4), INTENT(IN) :: mode

  INTEGER (KIND=Ki4) :: i, l, k, ii, nn, inc, item, nfax, istop
...
SUBROUTINE fftrig (n, mode)
  ...
  INTEGER (KIND=Ki4) :: i, l, la, nh, nn, imode

  REAL (KIND=Kr8) :: pi, del, angle

  pi=2.0_Kr8*ASIN(1.0_Kr8)
  imode=iabs(mode)
  nn=n
...
SUBROUTINE fft991 (a, work, inc, jump, n, lot, isign)
  ...
  INTEGER (KIND=Ki4), INTENT(IN) :: inc, jump, n, lot, isign

  REAL (KIND=Kr8), DIMENSION(:), INTENT(INOUT) :: a
  REAL (KIND=Kr8), DIMENSION(:), INTENT(INOUT) :: work

  INTEGER (KIND=Ki4) :: i, j, k, l, m, ia, ib, la, nx, nh
  INTEGER (KIND=Ki4) :: ink, igo, nfax, ibase, jbase
...
SUBROUTINE fft99a (a, work, inc, jump, n, lot)
  ...
  INTEGER (KIND=Ki4), INTENT(IN) :: inc, jump, n, lot

  REAL (KIND=Kr8), DIMENSION(:), INTENT(IN)  :: a
  REAL (KIND=Kr8), DIMENSION(:), INTENT(OUT) :: work

  INTEGER (KIND=Ki4) :: l, k, ia, ib, ja, jb, nh, nx, ink
  INTEGER (KIND=Ki4) :: iabase, ibbase, jabase, jbbase
...
!cdir nodep
     DO l=1,lot
        work(ja)=2.0_Kr8*a(ia)
        work(ja+1)=-2.0_Kr8*a(ia+inc)
        ia=ia+jump
        ja=ja+nx
     END DO
  END IF
...
SUBROUTINE fft99b (work, a, inc, jump, n, lot)
  ...
  REAL (KIND=Kr8), DIMENSION(:), INTENT(IN)  :: work
  REAL (KIND=Kr8), DIMENSION(:), INTENT(OUT) :: a

  INTEGER (KIND=Ki4) :: l, k, ia, ib, ja, jb, nh, nx, ink
  INTEGER (KIND=Ki4) :: iabase, ibbase, jabase, jbbase

  REAL (KIND=Kr8) :: c, s, scale

  nh=n/2
  nx=n+1
  ink=inc+inc
  scale=1.0_Kr8/REAL(n)
  ia=1
  ib=2
  ja=1
  jb=n*inc+1
...
SUBROUTINE vpassm (a, b, c, d, inc1, inc2, inc3, inc4, lot, n, ifac, la)
  ...
  INTEGER (KIND=Ki4), INTENT(IN) :: inc1, inc2, inc3, inc4, lot, n, ifac, la

  REAL (KIND=Kr8), DIMENSION (:), INTENT(IN)  :: a, b
  REAL (KIND=Kr8), DIMENSION (:), INTENT(OUT) :: c, d

  INTEGER (KIND=Ki4) :: i, j, k, l, m
  INTEGER (KIND=Ki4) :: ia, ja, ib, jb, kb, ic, jc, kc, id, jd, kd, ie, je, ke
  INTEGER (KIND=Ki4) :: igo, ijk, la1
  INTEGER (KIND=Ki4) :: iink, jink, jump, ibase, jbase

  REAL (KIND=Kr8) :: c1, s1,c2, s2, c3, s3, c4, s4
  REAL (KIND=Kr8) :: wka, wkb, wksina, wksinb, wkaacp, wkbacp, wkaacm, wkbacm

  INTEGER (KIND=Ki4), SAVE :: ifr=0

  REAL (KIND=Kr8), SAVE :: radi,sin60,sin36,sin72,cos36,cos72

  DATA ifr/0/
  IF (ifr == 0) THEN
     radi=ATAN(1.0_Kr8)/45.0_Kr8
     sin60=SIN(60.0_Kr8*radi)
     sin36=SIN(36.0_Kr8*radi)
     sin72=SIN(72.0_Kr8*radi)
     cos36=COS(36.0_Kr8*radi)
     cos72=COS(72.0_Kr8*radi)
     ifr=1
  END IF
  m=n/ifac
  iink=m*inc1
...
!cdir  nodep
        DO ijk=1,lot
           ...
           c(ja+j)=a(ia+i)+wka
           d(ja+j)=b(ia+i)+wkb
           c(jb+j)=(a(ia+i)-0.5_Kr8*wka)-wksinb
           c(jc+j)=(a(ia+i)-0.5_Kr8*wka)+wksinb
           d(jb+j)=(b(ia+i)-0.5_Kr8*wkb)+wksina
           d(jc+j)=(b(ia+i)-0.5_Kr8*wkb)-wksina
           i=i+inc3
           j=j+inc4
        END DO
        ibase=ibase+inc1
...
!cdir nodep
           DO ijk=1,lot
              ...
              c(ja+j)=a(ia+i)+wka
              d(ja+j)=b(ia+i)+wkb
              c(jb+j)=c1*((a(ia+i)-0.5_Kr8*wka)-wksinb) &
                     -s1*((b(ia+i)-0.5_Kr8*wkb)+wksina)
              d(jb+j)=s1*((a(ia+i)-0.5_Kr8*wka)-wksinb) &
                     +c1*((b(ia+i)-0.5_Kr8*wkb)+wksina)
              c(jc+j)=c2*((a(ia+i)-0.5_Kr8*wka)+wksinb) &
                     -s2*((b(ia+i)-0.5_Kr8*wkb)-wksina)
              d(jc+j)=s2*((a(ia+i)-0.5_Kr8*wka)+wksinb) &
                     +c2*((b(ia+i)-0.5_Kr8*wkb)-wksina)
              i=i+inc3
              j=j+inc4
           END DO
           ibase=ibase+inc1
...
!cdir nodep
           DO ijk=1,lot
              ...
              d(ja+j)=wkbacp+(b(ib+i)+b(id+i))
              c(jc+j)= &
                   c2*(wkaacp-(a(ib+i)+a(id+i))) &
                   -s2*(wkbacp-(b(ib+i)+b(id+i))) 
                   d(jc+j)= &
                   s2*(wkaacp-(a(ib+i)+a(id+i))) &
                   +c2*(wkbacp-(b(ib+i)+b(id+i)))
              c(jb+j)= &
                   c1*(wkaacm-(b(ib+i)-b(id+i))) &
                   -s1*(wkbacm+(a(ib+i)-a(id+i)))
              d(jb+j)= &
                   s1*(wkaacm-(b(ib+i)-b(id+i))) &
                   +c1*(wkbacm+(a(ib+i)-a(id+i)))
              c(jd+j)= &
                   c3*(wkaacm+(b(ib+i)-b(id+i))) &
                   -s3*(wkbacm-(a(ib+i)-a(id+i)))
              d(jd+j)= &
                   s3*(wkaacm+(b(ib+i)-b(id+i))) &
...
!cdir nodep
           DO ijk=1,lot
              ...
              d(je+j)=s4*((a(ia+i)+cos72*(a(ib+i)+a(ie+i))-cos36* &
                   ...
                   -(sin72*(a(ib+i)-a(ie+i))+sin36*(a(ic+i)- &
                   a(id+i))))
