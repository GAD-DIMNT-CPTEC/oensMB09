MODULE m_Vertical_Interpolation

  USE m_Parameters, ONLY: Ki4, Kr8, Gamma, g, Rd, Lc, Rv

  IMPLICIT NONE
 
  PRIVATE

  PUBLIC :: Vert_Sigma_Inter

CONTAINS


SUBROUTINE Vert_Sigma_Inter (Im, Jm, Km1, Km2, Nt, &
                             p1, u1, v1, t1, q1, &
                             p2, u2, v2, t2, q2)
  ...
  INTEGER (KIND=Ki4), INTENT (IN) :: Im, Jm, Km1, Km2, Nt

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km1), INTENT (IN) :: p1, u1, v1, t1

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km1,Nt), INTENT (IN) :: q1

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km2), INTENT (IN) :: p2

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km2), INTENT (OUT) :: u2, v2, t2

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km2,Nt), INTENT (OUT) :: q2

  INTEGER (KIND=Ki4) :: k, i, n

  REAL (KIND=Kr8) :: dltdz, dlpvdrt, dz

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km1) :: z1

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km2) :: z2

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km1,3+Nt) :: c1

  REAL (KIND=Kr8), DIMENSION (Im*Jm,Km2,3+Nt) :: c2
...
END SUBROUTINE Vert_Sigma_Inter
...
SUBROUTINE terp3 (Im, Km1, Km2, Nm, z1, q1, z2, q2)
  ...
  INTEGER (KIND=Ki4), INTENT (IN) :: Im, Km1, Km2, Nm

  REAL (KIND=Kr8), DIMENSION (Im,Km1), INTENT (IN) :: z1

  REAL (KIND=Kr8), DIMENSION (Im,Km2), INTENT (IN) :: z2

  REAL (KIND=Kr8), DIMENSION (Im,Km1,Nm), INTENT (IN) :: q1

  REAL (KIND=Kr8), DIMENSION (Im,Km2,Nm), INTENT (OUT) :: q2

  INTEGER (KIND=Ki4) :: k1, k2, i, n

  REAL (KIND=Kr8) :: z1a, z1b, z1c, z1d, z2s, q1a, q1b, q1c, q1d, q2s

  INTEGER (KIND=Ki4), DIMENSION (Im,Km2) :: k1s

  REAL (KIND=Kr8), DIMENSION (Im) :: ffa, ffb, ffc, ffd
...
SUBROUTINE rsearch (Im, Km1, Km2, z1, z2, l2)
  ...
  INTEGER (KIND=ki4), INTENT(IN) :: Im, Km1, Km2

  REAL (KIND=kr8), DIMENSION (Im,Km1), INTENT(IN) :: z1

  REAL (KIND=kr8), DIMENSION (Im,Km2), INTENT(IN) :: z2

  INTEGER (KIND=ki4), DIMENSION (Im,Km2), INTENT(OUT) :: l2

  INTEGER (KIND=ki4) :: i, k2

  INTEGER (KIND=ki4), DIMENSION (Km2) :: indx, rc
  ...
  DO i=1,Im
    ...
    WRITE (0, '(/,A)') ' Warnning: '
    WRITE (0, '(A)')   ' Input Coordinate is Monotonically Descending'
    WRITE (0, '(A)')   ' The Implemented Binary Search Does Not Allowed That'
    WRITE (0, '(A,/)') ' Stopping Computation at SUBROUTINE rsearch '
    STOP
...
SUBROUTINE bsrch (n, m, x, y, indx, rc)
  ...
  INTEGER (KIND=ki4), INTENT (IN) :: m, n

  REAL (KIND=kr8), DIMENSION (n), INTENT (IN) :: x
  REAL (KIND=kr8), DIMENSION (m), INTENT (IN) :: y

  INTEGER (KIND=ki4), DIMENSION (m), INTENT (OUT) :: indx, rc

  INTEGER (KIND=ki4) :: i, j, mi, mf, m2
...
END MODULE m_Vertical_Interpolation
