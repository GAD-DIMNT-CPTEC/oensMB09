...
CONTAINS
  ...
  !
  ! Subroutine: cliradintf (Clirad Interface)
  !
  ! $Author: panetta $
  ! $Date: 2007/07/20 13:58:43 $
  ! $Revision: 1.5 $
  ...
  !     COMPRESSES TWO-SIZE INPUT ARRAYES TO ONE-SIZE ARRAYES IN
  !       DAYTIME LATITUDE GRID POINTS (DLGP)
  ...
  !
  !
  ! ACRONYMS: CDLGP...CLOUDY DAYTIME LATITUDE GRID POINTS
  ...
  !
  !
  ! Modifications: T.Tarasova (????)
  !     H.Barbosa (from 11 may 2005) - attempt to get the
  !       same results with more than one processor.
  !
  !
  SUBROUTINE cliradintf(&
       imax  ,kmax  ,nls   ,noz   ,icld  ,s0 ,inalb  ,&
    ...
    ! input variables
    INTEGER, INTENT(IN) :: imax,kmax,nls,icld,inalb
    INTEGER(KIND=i8), INTENT(IN) :: imask (imax) !sib vegetation mask

    LOGICAL, INTENT(in) :: noz

    REAL(KIND=r8), INTENT(in) :: s0
    REAL(KIND=r8), INTENT(in), DIMENSION(imax) :: alvdf,alndf,cosz,tsea,gps
    REAL(KIND=r8), INTENT(in), DIMENSION(imax,kmax) :: pl20,dpl,tl,ql,o3l,cld
    REAL(KIND=r8), INTENT(in), DIMENSION(imax,kmax) :: clu,clwp,fice,rei,rel

    !hmjb It is set to inout because of the possibility of inalb.ne.2
    !     In this case they will be calculated.
    REAL(KIND=r8), INTENT(inout), DIMENSION(imax) :: alvdr,alndr
    ! output variables
    REAL(KIND=r8), INTENT(out), DIMENSION(imax) :: swinc,dswclr
    REAL(KIND=r8), INTENT(out), DIMENSION(imax) :: dswtop,ssclr,ss
    REAL(KIND=r8), INTENT(out), DIMENSION(imax) :: radvbl,radvdl,radnbl,radndl
    REAL(KIND=r8), INTENT(out), DIMENSION(imax) :: radvbc,radvdc,radnbc,radndc
    REAL(KIND=r8), INTENT(out), DIMENSION(imax,kmax) :: aslclr,asl,taud

    ! local  variables
    INTEGER i,k
    INTEGER :: nsol ! NUMBER OF SOLAR LATITUDE GRID POINTS (COSZ>0.01)

    REAL(KIND=r8):: scosz (imax) ! DOWNWARD FLUX AT TOP IN DLGP
    REAL(KIND=r8):: cmu   (imax) ! COSINE OF SOLAR ZENITH ANGLE IN DLGP
    INTEGER  :: dmask (imax) ! sib-mask in DLGP
    !
    !     Downward ground fluxes in dlgp
    !
    REAL(KIND=r8):: rvbl  (imax) ! VISIBLE, BEAM, CLEAR
    REAL(KIND=r8):: rvdl  (imax) ! VISIBLE, DIFFUSE, CLEAR
    REAL(KIND=r8):: rnbl  (imax) ! NearIR, BEAM, CLEAR
    REAL(KIND=r8):: rndl  (imax) ! NearIR, DIFFUSE, CLEAR
    REAL(KIND=r8):: rvbc  (imax) ! VISIBLE, BEAM, CLOUDY
    REAL(KIND=r8):: rvdc  (imax) ! VISIBLE, DIFFUSE, CLOUDY
    REAL(KIND=r8):: rnbc  (imax) ! NearIR, BEAM, CLOUDY
    REAL(KIND=r8):: rndc  (imax) ! NearIR, DIFFUSE, CLOUDY
    REAL(KIND=r8):: sl (imax) ! NET GROUND CLEAR FLUX IN DLGP
    REAL(KIND=r8):: sc (imax) ! NET GROUND CLOUDY FLUX IN DLGP
    !
    !     Other fluxes
    !
    REAL(KIND=r8):: dsclr (imax)        ! ABSORPTION OF CLEAR ATMOSPHERE AND GROUND
    REAL(KIND=r8):: dscld (imax)        ! DOWNWARD FLUX AT TOP IN DLGP
    REAL(KIND=r8):: aclr  (imax,kmax+1) ! ABSORPTION IN CLEAR ATM.
    REAL(KIND=r8):: acld  (imax,kmax+1) ! HEATING RATE (CLOUDY)
    !
    !     Albedo in DLGP
    !
    REAL(KIND=r8):: agv   (imax)        ! GROUND VISIBLE DIFFUSE ALBEDO IN DLGP
    REAL(KIND=r8):: agn   (imax)        ! GROUND NEAR IR DIFFUSE ALBEDO IN DLGP
    REAL(KIND=r8):: rsurfv(imax)        ! GROUND VISIBLE BEAM ALBEDO IN DLGP
    REAL(KIND=r8):: rsurfn(imax)        ! GROUND NEAR IR BEAM ALBEDO IN DLGP
    !
    !     Atmospheric state in DLGP
    !
    REAL(KIND=r8):: ps (imax)        ! surface pressure in DLGP (mb)
    REAL(KIND=r8):: ta (imax,kmax+1) ! LAYER TEMPERATURE IN DLGP
    REAL(KIND=r8):: wa (imax,kmax+1) ! LAYER SPECIFIC HUMIDITY IN DLGP
    REAL(KIND=r8):: oa (imax,kmax+1) ! LAYER OZONE MIXING RATIO IN DLGP
    REAL(KIND=r8):: pu (imax,kmax+2) ! PRESSURE AT BOTTOM OF LAYER IN DLGP
    REAL(KIND=r8):: dp (imax,kmax+1) ! PRESSURE DIFFERENCE IN DLGP
    !
    !     Clouds in DLGP
    !
    REAL(KIND=r8):: css   (imax,kmax+1) ! LARGE SCALE CLOUD AMOUNT IN DLGP
    REAL(KIND=r8):: ccu   (imax,kmax+1) ! CUMULUS CLOUD AMOUNT IN DLGP
    REAL(KIND=r8):: e0 (imax,kmax+1) ! Cloud optical depth
    REAL(KIND=r8):: tauxcl(imax,kmax)   ! extinction optical depth of liquid phase
    REAL(KIND=r8):: tauxci(imax,kmax)   ! extinction optical depth of ice phase
    REAL(KIND=r8):: frcice(imax,kmax+1) ! Fraction of cloud water in the form of ice
    REAL(KIND=r8):: radliq(imax,kmax+1) ! Ice particle Effective Radius (microns)
    REAL(KIND=r8):: radice(imax,kmax+1) ! Liquid particle Effective Radius (microns)

    REAL(KIND=r8):: tauk (imax,kmax+1) ! total optical depth with k=1 at top
    !
    !     Logical and working vectors
    !
    INTEGER  :: litx  (imax) ! NUMBERS OF DLGP IN ALL LAYERS
    INTEGER  :: listim(imax) ! =1,2,3...IMAX*(KMAX+1)
    LOGICAL  :: bitx  (imax) ! TRUE IN SOLAR LATITUDE GRID POINTS
    LOGICAL  :: bitc  (imax,kmax+1) ! Working logical
    ...
    !
    ! Set array listim = i, WHEN I=1,imax*(kmax+2)
    !
    FORALL (I=1:imax) listim(I)=I
    !
    ! set bits for daytime grid points
    ! BITX=.TRUE. IF COSZ>DUM(1)....0.01
    !
    bitx(1:imax)=cosz(1:imax).ge.0.01e0_r8
    !
    ! Calculate nsol = number of daytime latitude grid points
    !
    nsol=COUNT(bitx(1:imax))
    !
    ...
    !
    litx(1:nsol) = PACK(listim(1:imax), bitx(1:imax))
    !
    !  Transform  two-size  input arrays:
    !        pl20(imax,kmax),dpl,tl,ql,cld,clu
    !  in two-size arrays:
    !        pu(nsol,kmax+2),dp(nsol,kmax+1),ta,wa,css,ccu
    !  in daytime latitude grid points at all levels
    !
    DO i=1,nsol
       pu (i,3:kmax+2)=pl20(litx(i),1:kmax)
       dp (i,2:kmax+1)=dpl (litx(i),1:kmax)
       ta (i,2:kmax+1)=tl  (litx(i),1:kmax)
       wa (i,2:kmax+1)=max(0.1e-22_r8,ql(litx(i),1:kmax))
       oa (I,2:kmax+1)=max(0.1e-9_r8,o3l(litx(i),1:kmax))
       css(i,2:kmax+1)=cld (litx(i),1:kmax)
       ccu(i,2:kmax+1)=clu (litx(i),1:kmax)
       ps(i)=gps(litx(i))
    ENDDO
    ...
    !
    FORALL (I=1:nsol,litx(I).le.imax)
       agv  (I) = alvdf (litx(I))
       agn  (I) = alndf (litx(I))
       cmu  (I) = cosz  (litx(I))
    ...
    ENDDO
    !
    ! Transform scosz(nsol) to swinc(imax) at all lgp
    !
    ...
    ! Instead of occupying 1..imax, they cover only the range 1..nsol
    !
    ...
    IF (icld.eq.1) THEN
       ...
       e0(1:nsol,:) = 0.05_r8
       bitc(1:nsol,:) = (ta(1:nsol,:).lt.253.0_r8).and.(ccu(1:nsol,:).eq.0.0_r8)

       ! IF BITC=.TRUE. EO=0.025_r8
       WHERE (bitc(1:nsol,:)) e0(1:nsol,:)=0.025_r8
    ...
    ELSE IF(icld.eq.4) THEN
       ...
       DO k=1,kmax
          DO i=1,imax
             ...
             IF(tsea(i).gt.0.0_r8) THEN
                taud(i,k)=0.70_r8*(tauxcl(i,k)+tauxci(i,k))
             ELSE
                taud(i,k)=1.0_r8*(tauxcl(i,k)+tauxci(i,k))
             ENDIF
          ENDDO
       ENDDO
       ...
       DO i=1,nsol
          tauk  (i,2:kmax+1) = taud(litx(i),kmax:1:-1)
          frcice(i,2:kmax+1) = fice(litx(i),kmax:1:-1)
          radice(i,2:kmax+1) = rei (litx(i),kmax:1:-1)
          radliq(i,2:kmax+1) = rel (litx(i),kmax:1:-1)
       ENDDO
    ...
    DO i=1,nsol
       aslclr(litx(i),1:kmax)=aclr(i,2:kmax+1)
       asl   (litx(i),1:kmax)=acld(i,2:kmax+1)
    ENDDO
  ...
  !kmax
  !
  !puu(imax*(kmax+2))....level pressures mb       pl
  !taa(imax*(kmax+1))....layer temp in K       ta
  !waa(imax*(kmax+1))....layer specific humidity g/g      wa
  !oaa(imax*(kmax+1))....layer ozone concentration g/g    oa
  !tauc(imax*(kmax+1))...cloud optical depth
  !css(imax*(kmax+1))....cloud amount       fcld
  !cmu(imax).............cosine solar zenith angle        cosz
  !rsurfv(imax)..........Vis Beam Albedo       rsuvbm
  !agv(imax).............Vis Diffuse Albedo       rsuvdf
  !rsurfn(imax)..........Nir Beam Albedo       rsirbm
  !agn(imax).............Nir Diffuse Albedo       rsirdf
  ! psc   =  surface pressure   (mb)
  ! Outputs:
  !  dscld(imax)    ABSORPTION IN THE CLOUDY ATMOSPHERE AND AT THE GROUND
  !  sc(imax)    ABSORPTION AT THE GROUND IN CLOUDY CASE
  !acld(imax*(kmax+1)) HEATING RATE (CLOUDY) in K/sec
  !rvbc(imax)    VISIBLE BEAM  FLUXES (CLOUDY)
  !rvdc(imax)    VISIBLE DIFFUSE FLUXES (CLOUDY)
  !rnbc(imax)    NEAR-IR BEAM  FLUXES (CLOUDY)
  !rndc(imax)    NEAR-IR DIFFUSE FLUXES  (CLOUDY)
  !
  !
  !
  !
  SUBROUTINE cloudy(s0,rco2,m,np,&
    ...
    IF (icld.eq.1) THEN
       taucld(:,:,1)=0.0_r8
       taucld(:,:,2)=tauc
       reff(:,:,1) = 80.0_r8   ! ice particles
       reff(:,:,2) = 5.25_r8   ! water particles
    ELSEIF (icld.eq.4) THEN
       taucld(:,:,1)=tauc*fice
       taucld(:,:,2)=tauc*(1.0_r8-fice)
       reff(:,:,1) = rei
       reff(:,:,2) = rel
    ELSE
       WRITE(nfprt,*) 'error! icld must be 1 or 4 with Clirad-sw-m '
    ...
    taucld(:,:,3)= 0.0_r8 ! no droplets
    reff(:,:,3)  = 0.0_r8 ! no droplets
    fcld=csscgp ! cloud field
    ...
    DO i=1,m
       aotb_c(i,:)=asolc*dzint(i,:)*hzmask(i,:)/2.0_r8
       aotb_m(i,:)=asolm*dzint(i,:)*hzmask(i,:)/2.0_r8
       !need more testing
