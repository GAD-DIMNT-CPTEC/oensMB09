...
CONTAINS
  ...
  SUBROUTINE GrpComp(&
    ...
    REAL(KIND=r8),    INTENT(INOUT) :: vmax   (ibMax)
  ...
  SUBROUTINE TimeFilterStep1(fa, fb, jbFirst, jbLast)
    ...
    DO jb = jbFirst, jbLast
       DO k = 1, kMax
          DO ib = 1, ibMaxPerJB(jb)
             fgtmpmm1(ib,k,jb) = fgtmpm   (ib,k,jb)                        !snf
             fgtmpmm2(ib,k,jb) = fgtmp    (ib,k,jb)                          !snf
  ...
  SUBROUTINE GlobConservation(jFirst, jLast, jbFirst, jbLast, inew, init)
    ...
    DO j=jFirst,jLast
       s = 0.0      
       DO i=1,imaxperj(j)
          s = s + fgps(ibPerIJ(i,j),jbPerIJ(i,j))
       ENDDO
    ...
    ENDDO
    CALL MPI_ALLGATHERV(massconsrv(myfirstlat),myjmax,MPI_DOUBLE_PRECISION, &
                        massconsrv,nlatsinproc(0),displ(0),MPI_DOUBLE_PRECISION, & 
                        MPI_COMM_WORLD, ierr)
    totmass = 0.
    DO j=1,jmax
       totmass = totmass + massconsrv(j)
    ENDDO
    write(*,*) ' computed total mass ',totmass
    !$OMP END SINGLE
    IF (init.ne.1) THEN
       ...
       DO j=jFirst,jLast
          ...
          DO k =1,kmax
             ...
             DO i=1,imaxperj(j)
                ib = ibPerIJ(i,j)
                jb = jbPerIJ(i,j)
!               s = s + fgps(ib,jb)*fgpass_scalars(ib,k,jb,ns,inew)   
                s = s + fgpass_scalars(ib,k,jb,ns,inew)   
             ENDDO
             s1 = s1 + s * del(k)
          ENDDO
          fconsrv(ns,j) = s1 * cel_area(j)
       ENDDO
    ENDDO
    !$OMP BARRIER
    !$OMP SINGLE
    displ(0) = 0
    DO i=1,maxnodes-1
       displ(i) = displ(i-1) + nlatsinproc(i-1)*nscalars
    ENDDO
    CALL MPI_ALLGATHERV(fconsrv(1,myfirstlat),myjmax*nscalars,MPI_DOUBLE_PRECISION, &
                     fconsrv,nlatsinproc(0)*nscalars,displ(0),MPI_DOUBLE_PRECISION, & 
                     MPI_COMM_WORLD, ierr)
    DO ns=1,nscalars
       totmas(ns) = 0.
       DO j=1,jmax
