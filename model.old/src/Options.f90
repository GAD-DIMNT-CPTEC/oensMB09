...
MODULE Options
  ...
  CHARACTER(LEN=200), PUBLIC           :: fNameUnitTb
...
CONTAINS 
  ...
  SUBROUTINE InitNamModel() 
    ...
    NAMELIST /MODEL_IN/slagr,nlnminit,diabatic,eigeninit, &
                       ...
                       GenRestFiles,rmRestFiles, MasCon
  ...
  SUBROUTINE InitBlkdat
    ...
    rmRestFiles   =.FALSE.  ! rmRestFiles  --> .FALSE. ! Flag to remove restart files after
                                                       ! read them when .TRUE.
    MasCon        =.FALSE.  ! MasCon       --> .FALSE. ! Flag to do Mass Conservation (ln(ps))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE  coldwarm()
    ...
    fNameUnitTb = TRIM(path_in)//'aunits'
    ...
    IF(lexist) THEN
       PRINT*,'* The  ', TRIM(fNameUnitTb) ,' file exist *'
    ELSE
       PRINT*,'* The  ', TRIM(fNameUnitTb), ' file not exist*'
       STOP
    END IF
