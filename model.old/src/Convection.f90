...
CONTAINS
  ...
  SUBROUTINE rncl(len, pl, rno, clf)
    INTEGER, INTENT(IN ) :: len
    REAL,    INTENT(IN ) :: pl (len)  
    REAL,    INTENT(INOUT) :: rno(len)
    REAL,    INTENT(INOUT) :: clf(len)
...
999 FORMAT(' TLO ',I3,1X,I3,1X,10E12.5)
  ...
  SUBROUTINE gwater2( &
    ...
    REAL,    INTENT(inout  ) :: convts (ncols)
    REAL,    INTENT(inout  ) :: convcs (ncols)
    REAL,    INTENT(inout  ) :: convbs (ncols)
    REAL,    INTENT(inout  ) :: prct   (ncols)
    REAL,    INTENT(inout  ) :: prcc   (ncols)
    ...
    !    nDiag_toprec =  9  ! total precipiation
    !    nDiag_cvprec = 10  ! convective precipitation
    !    nDiag_lsprec = 11  ! large scale precipitation
    !    nDiag_snowfl = 12  ! snowfall
    !    nDiag_clheat = 33  ! convective latent heating
    !    nDiag_cmchan = 34  ! convective moisture change
    !    nDiag_lslhea = 35  ! large scale latent heating
    !    nDiag_lsmcha = 36  ! large scale moisture change
    !    nDiag_sclhea = 37  ! shallow convective latent heating
    !    nDiag_scmcha = 38  ! shallow convective moisture change
    !    nDiag_nshcrm = 70  ! negative specific humidity correction moisture src.
    ...
    CALL qnegat (fqn, fdqn, ftn, rdt, del, ncols, kmax)
    ...
    IF(iccon.EQ.'ARA '.OR.iccon.EQ.'YES '.OR.iccon.EQ.'KUO '& 
       ...
       DO i=1,ncols
          ...
          IF(dodia(nDiag_cvprec))bfr1(i)=fac2*rdt*1.0e3*rrr(i)
       END DO
       IF (ghl_local) THEN
          IF(dogrh(nGHis_cvprec,latco))CALL StoreGridHistory(rrr,nGHis_cvprec,latco,fac2*rdt*1.0e3)
       END IF
       ...
       IF(ghl_local)THEN
          IF(dogrh(nGHis_clheat,latco))CALL StoreGridHistory(dtwrk,nGHis_clheat,latco)
          IF(dogrh(nGHis_cvmosr,latco))CALL StoreGridHistory(dqwrk,nGHis_cvmosr,latco)
       END IF
    ...
    IF(ilcon.EQ.'YES ') THEN
       ...
       DO i=1,ncols
          ...
          IF(dodia(nDiag_toprec))bfr1(i)=fac2*rdt*1.0e3*rrr(i)
          IF(dodia(nDiag_lsprec))bfr3(i)=fac2*rdt*1.0e3*(rrr(i)-bfr3(i))
          IF(ghl_local.AND.dogrh(nGHis_snowfl,latco))bfrg(i)=0.0e0
          IF(dodia(nDiag_snowfl))bfr2(i)=0.0e0
          IF(0.35*ftn(i,1)+0.65*ftn(i,2).LE.273.2)THEN
             IF(ghl_local.AND.dogrh(nGHis_snowfl,latco))bfrg(i)=rrr(i)
             IF(dodia(nDiag_snowfl))bfr2(i)=fac2*rdt*1.0e3*rrr(i)
          ENDIF
       END DO
       IF(ghl_local)THEN
          IF(dogrh(nGHis_toprec,latco))CALL StoreGridHistory( rrr,nGHis_toprec,latco,fac2*rdt*1.0e3)    
          IF(dogrh(nGHis_snowfl,latco))CALL StoreGridHistory(bfrg,nGHis_snowfl,latco,fac2*rdt*1.0e3)
       ENDIF
