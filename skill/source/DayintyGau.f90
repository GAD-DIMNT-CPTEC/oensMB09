...
!*
      ...
      CHARACTER NFCT*10,NICN*15,NCLD*15,NPST*10,FNMANL*43,&
                DCLM*256,DPOS*256,TRUNC*10,ANL*7
      LOGICAL EXANL
      INTEGER MONL(12)
...
!*      ANL='GPOSAVN'
!*      TRUNC='T126L28'
!*      DPOS='/bangu/samfs/modoper/tempo/global/oens/avn/T126L28/GPOS/'
      !DPOS(57:60)=CYEAR
      !DPOS(61:61)='/'
      !DPOS(62:63)=CMONTH
      !DPOS(64:64)='/'
      !DPOS(65:66)=CDAY  
      !DPOS(67:67)='/'
      DPOS=TRIM(DPOS)//'/'//TRIM(CYEAR)//'/'//TRIM(CMONTH)//'/'//TRIM(CDAY)//'/'
      WRITE(*,*) "DPOS=",DPOS
      IDP=INDEX(DPOS//' ',' ')-1
!*      DCLM='/gfs/home3/modoper/tempo/global/oenspro/produtos/SKILL
!*     */CLIMA/'
      IDC=INDEX(DCLM//' ',' ')-1
      write(*,*)DCLM
!*
...
!TQ0042L028
      FNMANL(34:43)=TRUNC
!AMM      OPEN(97,FILE='filepst',STATUS='UNKNOWN')
!AMM      OPEN(98,FILE='filefct',STATUS='UNKNOWN')
      OPEN(99,FILE='fileinput.T126',STATUS='UNKNOWN')
      write(*,'(a)')DPOS(1:IDP)//FNMANL//'.grb'
      INQUIRE(FILE=DPOS(1:IDP)//FNMANL//'.grb',EXIST=EXANL)
!AMM      write(*,'(a)')DPOS(1:IDP)//FNMANL
!AMM      INQUIRE(FILE=DPOS(1:IDP)//FNMANL,EXIST=EXANL)
      IF (EXANL) THEN
      WRITE(99,'(A)')'avail'
      WRITE(99,'(A)')FNMANL//'.grb'
!AMM      WRITE(99,'(A)')FNMANL
!AMM      WRITE(99,'(A)')NCLD//'.ctl'
!AMM      WRITE(99,'(A)')NICN//'.ctl'
!AMM      DO NN=0,IDS-1
!AMM      WRITE(NF,'(I2.2)')NN
!AMM      WRITE(99,'(A)')NFCT//NF//'.ctl'
!AMM      ENDDO
!AMM      DO NN=0,IDS-1
!AMM      WRITE(NF,'(I2.2)')NN
!AMM      WRITE(99,'(A)')NPST//NF//'.ctl'
!AMM      ENDDO
      ELSE
      WRITE(99,'(A)')'unavl'
      WRITE(99,'(A)')FNMANL//'.grb'
...
!AMM      CALL FCTCTL(IDAY,MONTH,IYEAR,IUTC,NFCT,DPOS,IDP,ANL,TRUNC)
!AMM      CALL ICNCTL(IDAY,MONTH,IYEAR,IUTC,NICN,DPOS,IDP,ANL,TRUNC)
!AMM      CALL PSTCTL(IDAY,MONTH,IYEAR,IUTC,NPST,DPOS,IDP,ANL,TRUNC)
!*
...
!*
      ...
      CHARACTER NFCT*10,DPOS*256,ANL*7,TRUNC*10,NF*2
      INTEGER MONL(12)
      CHARACTER MON(12)*3
      DATA IDS /7/
      DATA MONL /31,28,31,30,31,30,31,31,30,31,30,31/
...
!AMM     *          IYEAR,MONTH,IDAY,IUTC,'P.fct.'//TRUNC//'.grb'
...
!AMM     *          IYEAR,MONTH,IDAY,IUTC,'P.fct.'//TRUNC//'.grb'
      ...
      WRITE(20,'(A)')'*'
      WRITE(20,'(A)')'VARS 42'
      WRITE(20,'(A)')'topo  0 132,1,0,0 '//'TOPOGRAPHY [m]'
      WRITE(20,'(A)')'lsmk  0  81,1,0,0 '//'LAND SEA MASK [0,1]'
      WRITE(20,'(A)')'pslc  0 135,1,0,0 '//'SURFACE PRESSURE [hPa]'
      WRITE(20,'(A)')'uves  0 192,1,0,0 '//'SURFACE ZONAL WIND (U) [m/s]'
      WRITE(20,'(A)')'vves  0 194,1,0,0 '//'SURFACE MERIDIONAL WIND (V) [m/s]'
      WRITE(20,'(A)')'psnm  0   2,102,0,0 '//'SEA LEVEL PRESSURE [hPa]'
      WRITE(20,'(A)')'tems  0 188,1,0,0 '//'SURFACE ABSOLUTE TEMPERATURE [K]'
      WRITE(20,'(A)')'umrs  0 226,1,0,0 '//'SURFACE RELATIVE HUMIDITY [no Dim]'
      WRITE(20,'(A)')'agpl  0  54,200,0,0 '//'INST. PRECIPITABLE WATER [Kg/m2]'
      WRITE(20,'(A)')'tsfc  0 187,1,0,0 '//'SURFACE TEMPERATURE [K]'
      WRITE(20,'(A)')'psmt  0 250,1,0,0 '//'TIME MEAN SURFACE PRESSURE [hPa]'
      WRITE(20,'(A)')'usmt  0 230,1,0,0 '//'TIME MEAN SURFACE ZONAL WIND (U) [m/s]'
      WRITE(20,'(A)')'vsmt  0 231,1,0,0 '//'TIME MEAN SURFACE MERIDIONAL WIND (V) [m/s]'
      WRITE(20,'(A)')'tsmt  0 232,1,0,0 '//'TIME MEAN SURFACE ABSOLUTE TEMPERATURE [K]'
      WRITE(20,'(A)')'rsmt  0 233,1,0,0 '//'TIME MEAN SURFACE RELATIVE HUMIDITY [no Dim]'
      WRITE(20,'(A)')'prec  0  61,1,0,0 '//'TOTAL PRECIPITATION [Kg/m2/day]'
      WRITE(20,'(A)')'prcv  0  63,1,0,0 '//'CONVECTIVE PRECIPITATION [Kg/m2/day]'
      WRITE(20,'(A)')'cbnv  0  71,3,0,0 '//'CLOUD COVER [0-1]'
      WRITE(20,'(A)')'role  0 114,8,0,0 '//'OUTGOING LONG WAVE AT TOP [W/m2]'
      WRITE(20,'(A)')'mask 17 137,100 '//'MASK [-/+]'
      WRITE(20,'(A)')'uvel 17  33,100 '//'ZONAL WIND (U) [m/s]'
      WRITE(20,'(A)')'vvel 17  34,100 '//'MERIDIONAL WIND (V) [m/s]'
      WRITE(20,'(A)')'omeg 17  39,100 '//'OMEGA [Pa/s]'
      WRITE(20,'(A)')'divg 17  44,100 '//'DIVERGENCE [1/s]'
      WRITE(20,'(A)')'vort 17  43,100 '//'VORTICITY [1/s]'
      WRITE(20,'(A)')'fcor 17  35,100 '//'STREAM FUNCTION [m2/s]'
      WRITE(20,'(A)')'potv 17  36,100 '//'VELOCITY POTENTIAL [m2/s]'
      WRITE(20,'(A)')'zgeo 17   7,100 '//'GEOPOTENTIAL HEIGHT [gpm]'
      WRITE(20,'(A)')'temp 17  11,100 '//'ABSOLUTE TEMPERATURE [K]'
      WRITE(20,'(A)')'umrl 17  52,100 '//'RELATIVE HUMIDITY [no Dim]'
      WRITE(20,'(A)')'umes 17  51,100 '//'SPECIFIC HUMIDITY [kg/kg]'
      WRITE(20,'(A)')'mkmt 17 240,100 '//'TIME MEAN MASK [-/+]'
      WRITE(20,'(A)')'uvmt 17 255,100 '//'TIME MEAN ZONAL WIND (U) [m/s]'
      WRITE(20,'(A)')'vvmt 17 241,100 '//'TIME MEAN MERIDIONAL WIND (V) [m/s]'
      WRITE(20,'(A)')'ommt 17 236,100 '//'TIME MEAN DERIVED OMEGA [Pa/s]'
      WRITE(20,'(A)')'dvmt 17 237,100 '//'TIME MEAN DIVERGENCE [1/s]'
      WRITE(20,'(A)')'vtmt 17 254,100 '//'TIME MEAN VORTICITY [1/s]'
      WRITE(20,'(A)')'fcmt 17 249,100 '//'TIME MEAN STREAM FUNCTION [m2/s]'
      WRITE(20,'(A)')'pvmt 17 252,100 '//'TIME MEAN VELOCITY POTENTIAL [m2/s]'
      WRITE(20,'(A)')'tvmt 17 253,100 '//'TIME MEAN VIRTUAL TEMPERATURE [K]'
      WRITE(20,'(A)')'uemt 17 248,100 '//'TIME MEAN SPECIFIC HUMIDITY [kg/kg]'
      WRITE(20,'(A)')'vdcc 17 227,100 '//'VERTICAL DIST TOTAL CLOUD COVER [no Dim]'
      WRITE(20,'(A)')'ENDVARS'
!*
      CLOSE(20)
!*
...
!AMM     *          LYEAR,MN,LDAY,IUTC,'P.fct.'//TRUNC//'.grb'
!*
      RETURN
      END
!*
...
!*
      ...
      CHARACTER NICN*10,DPOS*256,ANL*7,TRUNC*10
      CHARACTER MON(12)*3
      DATA MON /'JAN','FEB','MAR','APR','MAY','JUN',&
                'JUL','AUG','SEP','OCT','NOV','DEC'/
!*
...
!AMM     *          IYEAR,MONTH,IDAY,IUTC,'P.icn.'//TRUNC//'.grb'
      WRITE(20,'(A)')'*'
      WRITE(20,'(A)')'OPTIONS YREV'
      WRITE(20,'(A)')'DTYPE GRIB'
...
!*
      INTEGER IDAY,MONTH,IYEAR,IUTC,&
             KYEAR,JDAY,MN,IDS,IDP,&
             NDAY,MDAY,LDAY,LMN,LYEAR
      CHARACTER NPST*10,DPOS*256,ANL*7,TRUNC*10,NF*2
      INTEGER MONL(12)
      CHARACTER MON(12)*3
      DATA IDS /7/
      DATA MONL /31,28,31,30,31,30,31,31,30,31,30,31/
...
!*
      WRITE(97,'(A,2(I4.4,3I2.2),A)') ANL,KYEAR,MN,NDAY,IUTC,&
                 KYEAR,MN,NDAY,IUTC,'P.icn.'//TRUNC
!AMM     *          KYEAR,MN,NDAY,IUTC,'P.icn.'//TRUNC//'.grb'
      WRITE(20,'(A,2(I4.4,3I2.2),A)')'DSET '//ANL,KYEAR,MN,NDAY,IUTC,&
                 KYEAR,MN,NDAY,IUTC,'P.icn.'//TRUNC
!AMM     *          KYEAR,MN,NDAY,IUTC,'P.icn.'//TRUNC//'.grb'
      WRITE(20,'(A)')'*'
      WRITE(20,'(A)')'OPTIONS YREV'
      ...
      WRITE(20,'(A)')'                      50  30  20  10   3'
      WRITE(20,'(A,I3,A,I2.2,A,I2.2,A,I4.4,A)')&
               'TDEF ',1,' LINEAR ',IUTC,'Z',NDAY,MON(MN),KYEAR,' 1DY'
      WRITE(20,'(A)')'*'
      WRITE(20,'(A)')'VARS 22'
      WRITE(20,'(A)')'topo  0 132,1,0,0 '//'TOPOGRAPHY [m]'
      WRITE(20,'(A)')'lsmk  0  81,1,0,0 '//'LAND SEA MASK [0,1]'
...
!*
      ...
      WRITE(97,'(A,2(I4.4,3I2.2),A)')ANL,IYEAR,MONTH,IDAY,IUTC,LYEAR,MN,LDAY,IUTC,'P.fct.'//TRUNC
!AMM     *          LYEAR,MN,LDAY,IUTC,'P.fct.'//TRUNC//'.grb'
      CLOSE(97)
!*
      RETURN
