...
#    print(minv)
  ...
  maxval = max(maxvals)
#  maxval = "{:10.4f}".format(max(maxvals))
#  maxval = "{0:.4e}".format(max(maxvals))
  print('Max:',maxval)

  minval = min(minvals)
#  minval = "{:10.4f}".format(min(minvals))
#  minval = "{0:.4e}".format(min(minvals))
  print('Min:',minval)
  ...
  y = np.arange(-0.2, 1.1, 0.1)
#  y = np.arange(float(minval)-0.1, 1.01, 0.1)
  ylabels = []
  for i in y:
    ylabels.append(i)
