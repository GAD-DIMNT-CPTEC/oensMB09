...
MODULE InputParameters
   ...
   INTEGER (KIND=i4), PARAMETER, PUBLIC :: nFmxI=11_i4, nFmxO=13_i4

   INTEGER (KIND=i4), PUBLIC :: nPTx, nFmx, MxGHsl, Imax, Jmax

   CHARACTER (LEN=3_i4), PUBLIC :: Lev

   CHARACTER (LEN=4_i4), PUBLIC :: Trunc

   CHARACTER (LEN=4_i4), PUBLIC :: GrdBox

   CHARACTER (LEN=7_i4), PUBLIC :: Resol
   ...
   CHARACTER (LEN=40_i4), PUBLIC :: TopogInp

   CHARACTER (LEN=40_i4), PUBLIC :: GrHDatIn

   CHARACTER (LEN=40_i4), PUBLIC :: GrHDatOut

   CHARACTER (LEN=40_i4), PUBLIC :: GrHPrfOut

   CHARACTER (LEN=40_i4), PUBLIC :: GrHLocOut

   CHARACTER (LEN=40_i4), PUBLIC :: GrHIdfOut

   CHARACTER (LEN=44_i4), PUBLIC :: GrHCtlOut

   CHARACTER (LEN=96_i4), PUBLIC :: DirTopog

   CHARACTER (LEN=96_i4), PUBLIC :: DirModel

   CHARACTER (LEN=96_i4), PUBLIC :: DirOut
...
SUBROUTINE InitParameters ()
   ...
   NAMELIST /InputDim/ Mend, Kmax, mUTC, DelT, TMean, &
                       LabelI, LabelF, Preffix, GrdBox, DirMain
   ...
   Preffix='SMT'       ! Preffix of File Names
   ...
   ! Resolution
   Trunc='T  '
   WRITE (Trunc(2_i4:4_i4), FMT='(I3.3)') Mend
   Lev='L  '
   WRITE (Lev(2_i4:3_i4), FMT='(I2.2)') Kmax
   Resol=Trunc//Lev

   ! Dates
   DateICn=LabelI//LabelI
   DateFct=LabelI//LabelF

   ! Directories
   DirTopog=TRIM(DirMain)//'model/dataout/'//Resol//'/ '
   DirModel=TRIM(DirMain)//'model/dataout/'//Resol//'/ '
   DirOut=TRIM(DirMain)//'pos/dataout/'//Resol//'/ '

   ! Input Files
   TopogInp='GFGH'//Preffix//DateFct//'F.top'//'.'//Resol
   GrHDirIn='GFGH'//Preffix//DateFct//'F.dir'//'.'//Resol
   GrHDatIn='GFGH'//Preffix//DateFct//'F.unf'//'.'//Resol

   ! Output Files (DirOut)
   GrHDatOut='GFGN'//Preffix//DateFct//'M.grh'//'.'//Resol
   GrHCtlOut='GFGN'//Preffix//DateFct//'M.grh'//'.'//Resol//'.ctl'
   GrHPrfOut='Preffix'//Preffix//DateFct//'.'//Resol
   GrHLocOut='Localiz'//Preffix//DateFct//'.'//Resol
   GrHIdfOut='Identif'//Preffix//DateFct//'.'//Resol

   OPEN (UNIT=10, STATUS='UNKNOWN',FORM='FORMATTED', &
         FILE=TRIM(DirModel)//GrhDirIn)
