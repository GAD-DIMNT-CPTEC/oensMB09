...
PROGRAM SnowClima
   ...
   USE InputParameters, ONLY : r4, r8, Undef, &
                               ...
                               Date, Exts, DirInp, DirOut, VarName, &
                               IcePoints, GrADS, &
                               InitInputParameters
   ...
   OPEN (UNIT=nfclm, FILE=TRIM(DirInp)//'/'//NameAlb//nLats, &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecIn, &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirInp)//'/'//NameAlb//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   OPEN (UNIT=nfout, FILE=TRIM(DirOut)//'/'//VarNameS//Date//Exts//nLats, &
         FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, ACTION='WRITE', &
         STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file: ', &
              TRIM(DirOut)//'/'//VarNameS//Date//Exts//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   IF (GrADS) THEN
      ...
      OPEN (UNIT=nfctl, FILE=TRIM(DirOut)//'/'//VarName//Date//Exts//nLats//'.ctl', &
            FORM='FORMATTED', ACCESS='SEQUENTIAL', ACTION='WRITE', &
            STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//'/'//VarName//Date//Exts//nLats//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
      WRITE (UNIT=nfctl, FMT='(A)') 'DSET '// &
            TRIM(DirInp)//'/'//VarNameS//Date//Exts//nLats
