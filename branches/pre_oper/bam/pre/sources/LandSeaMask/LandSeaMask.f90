...
PROGRAM LandSeaMask
   ...
   USE InputParameters, ONLY: r4, r8, Undef, &
                              ...
                              Imax, Jmax, Idim, Jdim, &
                              nLats, mskfmt, VarName, &
                              VarNameIn, VarNameOut, DirInp, &
                              DirOut, GrADS, InitInputParameters
   ...
   OPEN (FILE=TRIM(DirInp)//"/"//TRIM(VarNameIn)//'.dat', &
         UNIT=nfclm, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecIn, &
         ACTION='READ', STATUS='OLD', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirInp)//"/"//TRIM(VarNameIn)//'.dat', &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   DO j=1,Jdim
   ...
   OPEN (FILE=TRIM(DirOut)//"/"//VarName//nLats, &
   ...
   IF (GrADS) THEN
      ...
      OPEN (FILE=TRIM(DirOut)//"/"//TRIM(VarNameOut)//nLats//'.dat', &
            UNIT=nfoub, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//"/"//TRIM(VarNameOut)//nLats//'.dat', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
      ...
      OPEN (FILE=TRIM(DirOut)//"/"//TRIM(VarNameOut)//nLats//'.ctl', &
            UNIT=nfctl, FORM='FORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//"/"//TRIM(VarNameOut)//nLats//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
