...
PROGRAM TopoSpectral
   ...
   OPEN (FILE=TRIM(DirOut)//"/"//TRIM(VarNameT)//Trunc, &
         UNIT=nftpo, FORM='UNFORMATTED', ACCESS='SEQUENTIAL', &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//"/"//TRIM(VarNameT)//Trunc, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   OPEN (FILE=TRIM(DirOut)//"/"//TRIM(VarNameT)//'Rec'//nLats, &
            UNIT=nfout, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRec, &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
       WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//"/"//TRIM(VarNameT)//'Rec'//nLats, &
             ' returned IOStat = ', ios
       STOP  ' ** (Error) **'
   END IF
   ...
   IF (GrADS) THEN
      ...
      OPEN (FILE=TRIM(DirOut)//"/"//TRIM(VarNameT)//'Rec'//nLats//'.ctl', &
            UNIT=nfctl, FORM='FORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//"/"//TRIM(VarNameT)//'Rec'//nLats//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
