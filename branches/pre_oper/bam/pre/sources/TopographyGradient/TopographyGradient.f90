PROGRAM TopographyGradient
  ...
  CALL CreateLegTrans ()
   ...
   OPEN (FILE=TRIM(DirOut)//'/'//TRIM(VarNameT)//nLats, &
         UNIT=nfoub, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file: ', &
              TRIM(DirOut)//'/'//TRIM(VarNameT)//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   IF (GrADS) THEN
      ...
      IF (ios /= 0) THEN
         ...
         STOP  ' ** (Error) **'
      END IF
      WRITE (UNIT=nfctl, FMT='(A)') 'DSET '//TRIM(DirOut)//'/'//TRIM(VarNameT)//nLats
  ...
  WRITE(*,*)TRIM(DirOut)//'/'//TRIM(FileInp)
  WRITE(*,*)TRIM(DirOut)//'/'//TRIM(FileOut)
  OPEN(UNIT=nfinp, FILE=TRIM(DirOut)//'/'//TRIM(FileInp), &
                   FORM='UNFORMATTED', STATUS='OLD')

  INQUIRE (IOLENGTH=LRec) GrADSOut
  OPEN(UNIT=nfout, FILE=TRIM(DirOut)//'/'//TRIM(FileOut), &
 ...
 ! Meridional Gradient off Topography
  ...
  WRITE (UNIT=nfctl, FMT='(A)') 'dset '//TRIM(DirOut)//'/'//TRIM(FileOut)
