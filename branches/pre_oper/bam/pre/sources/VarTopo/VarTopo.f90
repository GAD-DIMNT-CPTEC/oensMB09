...
PROGRAM VarTopo
   ...
   USE InputParameters, ONLY: r4, r8, Undef, VarMin, VarMax, &
                              ...
                              Imax, Jmax, Idim, Jdim, &
                              nLats, VarNameT, VarNameV, &
                              DirInp, DirOut, VarName, VarNameG, GrADS, Linear, &
                              InitInputParameters
   ...
   ! OPEN (FILE=TRIM(DirInp)/"/"//TRIM(VarName)//'.dat', &
   OPEN (FILE=TRIM(DirInp)//"/"//TRIM(VarName)//'.dat', &
   ...
   OPEN (FILE=TRIM(DirOut)//"/"//VarNameV//nLats, &
         UNIT=nfoua, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//"/"//VarNameV//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   OPEN (FILE=TRIM(DirOut)//"/"//VarNameT//nLats, &
         UNIT=nfoub, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
         ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
   IF (ios /= 0) THEN
      WRITE (UNIT=nferr, FMT='(3A,I4)') &
            ' ** (Error) ** Open file ', &
              TRIM(DirOut)//"/"//VarNameT//nLats, &
            ' returned IOStat = ', ios
      STOP  ' ** (Error) **'
   END IF
   ...
   IF (GrADS) THEN
      ...
      OPEN (FILE=TRIM(DirOut)//"/"//TRIM(VarNameG)//nLats, &
            UNIT=nfouc, FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecOut, &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//"/"//TRIM(VarNameG)//nLats, &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
      FieldOut=REAL(TopoOutput,r4)
      ...
      OPEN (FILE=TRIM(DirOut)//"/"//TRIM(VarNameG)//nLats//'.ctl', &
            UNIT=nfctl, FORM='FORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//"/"//TRIM(VarNameG)//nLats//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
