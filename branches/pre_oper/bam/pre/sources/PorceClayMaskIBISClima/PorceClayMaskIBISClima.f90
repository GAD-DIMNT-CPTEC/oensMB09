...
PROGRAM PorceClayMaskIBISClima
   ...
   NAMELIST /InputDim/ Idim, Jdim,Layer, GrADS, DirInp, DirOut, DirBcs
   ...
   OPEN (UNIT=nfvgm, FILE=TRIM(DirOut)//'/'//TRIM(VarName)//'.dat', &
   ...
   IF (GrADS) THEN
      ...
      OPEN (UNIT=nfout, FILE=TRIM(DirOut)//'/'//TRIM(VarNameG)//'.dat', &
            FORM='UNFORMATTED', ACCESS='DIRECT', RECL=LRecGad, ACTION='WRITE', &
            STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//'/'//TRIM(VarNameG)//'.dat', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
      ...
      OPEN (UNIT=nfctl, FILE=TRIM(DirOut)//TRIM(VarNameG)//'.ctl', &
            FORM='FORMATTED', ACCESS='SEQUENTIAL', &
            ACTION='WRITE', STATUS='REPLACE', IOSTAT=ios)
      IF (ios /= 0) THEN
         WRITE (UNIT=nferr, FMT='(3A,I4)') &
               ' ** (Error) ** Open file ', &
                 TRIM(DirOut)//'/'//TRIM(VarNameG)//'.ctl', &
               ' returned IOStat = ', ios
         STOP  ' ** (Error) **'
      END IF
